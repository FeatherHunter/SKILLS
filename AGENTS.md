...
### 🖥 需要用户视觉确认的交付物 · 主动打开并提醒（2026-08-14 用户要求 · 长期生效）

**任何需要用户「看」的交付物（验收演示页 / HTML 渲染结果 / 视觉对比 / 截图产物等），交付时 agent 必须主动打开浏览器并明确提醒用户查看**，不得只给路径/链接让用户自己找：

- 打开方式：自包含 HTML 用 `Start-Process "<绝对路径>"`（默认浏览器）打开；需要服务的用托管后台任务起本地 HTTP 后打开 URL；能嵌入当前 GUI 时优先嵌入
- 打开后提醒内容：页面位置 + **看什么**（验收点清单）+ 怎么切双端（DevTools 设备模拟或缩窗）
- 适用范围：验收演示、视觉回归、UI/样式改动、帮助页/模板渲染结果等一切「用户看了才算数」的产物
