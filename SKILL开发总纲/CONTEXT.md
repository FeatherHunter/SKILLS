...
**HTML 单工铁律**:
HTML 是单工设备,永远单向渲染,不能反向触发 LLM。任何过程型 HTML 必须设计"复制指令"按钮,让用户的选择回到 AI。原则 10,最高优先级。
_Avoid_: 单向绑定、只读视图
...
**4 段式模板**:
HTML 模板的标准结构:首屏 HERO(标题 + KPI)/ 主体 BODY(分组折叠)/ 交互 INTERACT(轻 JS)/ 尾部 TAIL(复制指令 + 回顶)。5 状态必有:正常 / 空 / 缺数据 / 错误 / 离线。
_Avoid_: 布局、页面结构
