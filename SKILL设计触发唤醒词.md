...
### 来源文件
- `卡路里/SKILL.md` — 主定义 + 触发词速查表
- `卡路里/scripts/calorie_tracker.py` — 热量追踪
- `卡路里/scripts/exercise_tracker.py` — 运动追踪
- `卡路里/scripts/sleep_tracker.py` — 睡眠追踪
- `卡路里/scripts/fitness_goals.py` — 健身目标
- `卡路里/scripts/analysis.py` — 数据分析

### 唤醒词与功能映射

| 唤醒词 | 功能 | 来源 |
|--------|------|------|
| 吃了碗米饭、中午吃了biangbiang面 | 记录饮食 | `卡路里/SKILL.md:56` |
| 发营养成分表图片 | 图片识别 + 记录 | `卡路里/SKILL.md:56` |
| 删掉那条记录、删除ID 3 | 删除记录 | `卡路里/SKILL.md:57` |
| 今天吃了什么、今天吃了多少卡、今日摘要、list | 查看今日 | `卡路里/SKILL.md:58` |
| 最近7天吃了什么、这周热量多少 | 查询历史 | `卡路里/SKILL.md:59` |
| 查一下鸡胸肉的热量、可乐的营养成分 | 搜索食品 | `卡路里/SKILL.md:60` |
| 把这个存到食品库、添加营养成分表 | 添加食品到库 | `卡路里/SKILL.md:61` |
| 更新ID 1的热量 | 更新食品库 | `卡路里/SKILL.md:62` |
| 看看食品库里有什么 | 查看食品库 | `卡路里/SKILL.md:63` |
| 体重70公斤、今天69.5kg | 记录体重 | `卡路里/SKILL.md:64` |
| 体重历史、最近30天体重 | 体重历史 | `卡路里/SKILL.md:65` |
| 体重趋势、这周和上周比、体重波动 | 体重趋势/对比 | `卡路里/SKILL.md:66` |
| 离目标还差多久、按现在速度多久能到 | 目标进度 | `卡路里/SKILL.md:67` |
| 今天骑行了40分钟、做了30个俯卧撑 | 记录运动 | `卡路里/SKILL.md:68` |
| 运动摘要、运动趋势、运动类型、运动贡献 | 运动报表 | `卡路里/SKILL.md:69` |
| 热量趋势、这周吃了多少、热量缺口 | 热量分析 | `卡路里/SKILL.md:70` |
| 营养配比、蛋白质够吗、碳水是不是吃多了 | 营养配比 | `卡路里/SKILL.md:71` |
| 食物排行榜、低热量榜、频繁吃榜、高碳水榜、高蛋白榜 | 食物排行榜 | `卡路里/SKILL.md:72` |
| 每天吃1800卡、goal 2000 150 200 60 | 设置营养目标 | `卡路里/SKILL.md:73` |
| 设置俯卧撑目标、每天做50个俯卧撑 | 健身目标 | `卡路里/SKILL.md:74` |
| 昨晚睡了多久、睡眠7小时、昨晚11点睡的 | 睡眠记录 | `卡路里/SKILL.md:75` |
| dashboard、给我看看整体情况 | 综合报告 | `卡路里/SKILL.md:76` |
| 健康检查、lint、检查数据 | 健康检查 | `卡路里/SKILL.md:77` |
| 醒了、睡醒了、起床啦 | **起床唤醒**（自动记录睡眠） | `卡路里/SKILL.md:223-228` |

### 起床唤醒词处理流程 — `卡路里/SKILL.md:222-231`
1. 检测到唤醒词（"醒了"/"睡醒了"/"起床啦"）→ 查询录音机数据库
...
| 删除、删掉 | 备忘录 `SKILL.md:70` / 卡路里 `SKILL.md:57` | 需上下文消歧 |
...
| 最近 | 备忘录 `SKILL.md:79` / 饼干记账 `SKILL.md:58` / 卡路里 `SKILL.md:59` | 需上下文消歧 |
| 提醒 | 备忘录 `SKILL.md:88` / 共享单车提醒 `SKILL.md:11` | 备忘录优先（强制路由） |
| 健康检查、lint、检查数据 | 居家管家 `SKILL.md:114` / 卡路里 `SKILL.md:77` | 需上下文消歧 |
...
| 刚睡、22:30睡的 | 勋章 `SKILL.md:822` / 卡路里 `SKILL.md:75` / 作息管家 | 勋章触发颁奖，卡路里/作息管家触发睡眠记录——可能联动 |
| 称了体重 | 勋章 `SKILL.md:869` / 卡路里 `SKILL.md:64` | 勋章触发颁奖，卡路里触发体重记录——可能联动 |
| 跑了步、骑了车 | 勋章 `SKILL.md:885` / 卡路里 `SKILL.md:68` | 勋章触发颁奖，卡路里触发运动记录——可能联动 |
