/**
 * dsh-opencode-tui-theme v1.3.1 — 浏览器半（构建产物，勿手改）
...
function renderTheme(name, typography) {
  ...
  if (errors.length > 0) {
    console.warn('[dsh-opencode-tui-theme] ' + name + ' 有 ' + errors.length + ' 个色位解析失败:', errors)
  }
...
const STORAGE_KEY = 'dsh.opencode-tui-theme.v2'
  ...
  return function apply(ctx) {
    ...
    function applyStyle() {
      ...
      try {
        ...
        console.error('[dsh-opencode-tui-theme] 渲染失败，回退默认主题:', e)
        render = renderTheme(DEFAULT_STATE.theme, { mode: 'mono', size: 13, fontKey: 'JetBrains Mono' })
      }
      // 3) token 层（{light,dark} 同值 = 强制深色终端观感）
      tokenDispose = theme.overrideTokens('opencode-tui-theme', render.tokens)
      // 4) <style> 层
    ...
    ctx.effect(function () {
      return function () {
        ...
        try { if (globalThis.__opencodeTheme) delete globalThis.__opencodeTheme } catch (e) { /* 忽略 */ }
      }
