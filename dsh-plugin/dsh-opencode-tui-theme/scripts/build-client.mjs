...
async function main() {
  ...
  const ID = 'dsh-opencode-tui-theme'
  ...
  const pkgBundle = [
    '/**',
    ' * dsh-opencode-tui-theme v' + version + ' — 浏览器半（构建产物，勿手改）',
    ...
    // TypeError: {} is not a function（dsh-opencode-tui-theme v1.2.0 加载失败的根因）
    '    var __mods = {};',
    modules,
  ...
  const dynBundle = [
    '// dsh-opencode-tui-theme v' + version + ' — 动态版（构建产物，勿手改）',
  ...
  const host = [
    '/**',
    ' * dsh-opencode-tui-theme v' + version + ' — 宿主半（no-op）',
    ' *',
  ...
  const readme = await readFile(join(ROOT, 'README.md'), 'utf8').catch(() => '(README 缺失)')
