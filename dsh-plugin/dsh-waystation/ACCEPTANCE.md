...
> 用途：运行时验收清单。插件经多轮迭代（v9→v15）在带 cordis 工具的会话中加载并实操验收，
> 本文件按 **v15 实际形态** 回填勾选结果（2026-08-14）。
...
| 2.2 | 列表页 KPI 行 | 可接 / 占用 / 已关闭 计数 + 刷新按钮；**open 列表 map 项优先置顶**（v15，map 内部按 updatedAt 倒序，已关闭行保持时间序） | ☑ |
| 2.3 | 标签过滤 chips | 动态统计（GitHub 配置色）；**「全部」恒清空过滤并保持选中**（反复点击不进入空过滤态）；**chips 边框 = label 色深一档** | ☑ |
| 2.4 | 行级动作 | **按 label 三选一**：needs-triage→分流(/triage+URL) / bug→开始修复(/wayfinder+URL) / 其余 open→开始执行(/wayfinder+URL+流程指令)，点击均**复制 prompt**；按钮为 80% 小尺寸；**被阻塞行（open 阻塞者）→ 红色「被阻塞」标签（tooltip 列来源，点击跳所属 map 详情被阻塞分组）+ 动作按钮全部隐藏**（v15） | ☑ |
...
FAIL 项：无
  现象：
  处理：
验收人：FeatherHunter（多轮实操评审 + 2026-08-14 v14 全部执行批次 + v15 第二批验证）
日期：2026-08-14
```
