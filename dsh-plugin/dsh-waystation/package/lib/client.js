  ...
  factory: (require) => {
    ...
    exports.apply = function (ctx) {
      ...
      // ============================================================
        ...
        "newWayfinder": { version: 3, placeholders: ['repo'], use: '「+ 新建需求」按钮', zh: '/wayfinder\n请帮我处理一个需求（严格遵循 wayfinder 技能规则）：\n根据需求情况，选择 新增 / 复用 / 直接实现 之一：\n- 新增：全新需求 → 按规划契约建 map 或直接实现；\n- 复用：已有类似实现 / 技能 / 组件 → 优先复用，不重复造轮子；\n- 直接实现：需求小而明确 → 直接实现。\n仓库：{repo}\n\n先澄清需求：若对目标 / 范围 / 偏好有假设，先用 grilling 技能澄清。\n若目标较大：先按建图规划契约建 map（Destination + Notes + 规划表 + 票），不直接写码。\n需求描述：', en: '/wayfinder\nPlease handle a requirement (strictly follow the wayfinder skill rules):\nChoose one of add / reuse / directly implement based on the situation:\n- Add: a brand-new requirement → build a map per the planning contract, or implement directly;\n- Reuse: an existing similar implementation / skill / component exists → reuse it first, do not reinvent the wheel;\n- Directly implement: the requirement is small and clear → implement directly.\nRepo: {repo}\n\nClarify first: if you hold assumptions about the goal / scope / preferences, settle them with the grilling skill.\nIf the goal is large: build a map per the planning contract (Destination + Notes + plan + tickets) instead of coding directly.\nRequirement: ' },
