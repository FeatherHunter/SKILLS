...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    exports.apply = function (ctx) {
      ...
      styleEl.setAttribute('data-plugin', 'dsh-waystation')
      ...
      // ============================================================
        ...
        "newWayfinder": { version: 4, placeholders: ['repo'], use: '「+ 新建需求」按钮', zh: '/wayfinder\n请帮我处理一个需求（严格遵循 wayfinder 技能规则）。先判断它属于哪种情况（需求/map 维度）：\n- 新增：这是全新需求，之前没做过 → 按建图规划契约新建 map（Destination + Notes + 规划表 + 票）；\n- 复用：这个需求之前已经做过（已有 map / issue / 类似需求）→ 复用它，不重复建；\n- 直接实现：需求很小，当前对话直接实现即可，不用建大 map（一个 issue 甚至直接对话搞定）。\n仓库：{repo}\n\n先澄清需求：若对目标 / 范围 / 偏好有假设，先用 grilling 技能澄清。\n若目标较大：先按建图规划契约建 map（Destination + Notes + 规划表 + 票），不直接写码。\n需求描述：', en: '/wayfinder\nPlease handle a requirement (strictly follow the wayfinder skill rules). First decide which case it is (at the requirement/map level):\n- Add: this is a brand-new requirement never done before → build a new map per the planning contract (Destination + Notes + plan + tickets);\n- Reuse: this requirement has been done before (an existing map / issue / similar requirement) → reuse it, do not build a new one;\n- Directly implement: the requirement is small enough to implement right here in this conversation — no need for a big map (a single issue, or even just this conversation).\nRepo: {repo}\n\nClarify first: if you hold assumptions about the goal / scope / preferences, settle them with the grilling skill.\nIf the goal is large: build a map per the planning contract (Destination + Notes + plan + tickets) instead of coding directly.\nRequirement: ' },
