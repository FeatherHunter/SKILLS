...
**若PDF文件较大（>3MB），直接发给GLM-OCR会超时。正确做法：先截图再OCR。**

### 流程
```bash
# 1. 用PyMuPDF将PDF每页渲染为PNG（2x缩放）
python3 -c "
import pymupdf, os
doc = pymupdf.open('大PDF.pdf')
for i in range(1, 11):  # 先处理10页测试
    page = doc[i-1]
    pix = page.get_pixmap(matrix=pymupdf.Matrix(2, 2))
    pix.save(f'output/page_{i}.png')
"
...
API_KEY = '你的KEY'
for i in range(1, 11):
    with open(f'output/page_{i}.png', 'rb') as f:
    ...
    md = resp.json().get('md_results', '')
    with open(f'output/page_{i}.md', 'w', encoding='utf-8') as f:
        f.write(md)
    print(f'第{i}页完成')
    time.sleep(1)
"
...
### 关键参数
- `pymupdf.Matrix(2, 2)` — 2x缩放，保证OCR识别质量
- 每页PNG约200-400KB，比同等PDF小很多，API响应快
- `timeout=120` — 截图方案单张通常<10s
