#!/usr/bin/env python3
"""阶段3：读取layout_details_final.json，用python-docx构建Word文档 - V2修复版"""
...
class GLMOCRToWord:
    ...
    def convert(self, layout_details, output_path):
        """主转换函数"""
    ...
    def _parse_html_text(self, content):
        """解析HTML内容，提取纯文本和对齐方式"""
        if not content:
        ...
        return content, WD_ALIGN_PARAGRAPH.LEFT
    ...
    def _add_text(self, doc, content, bbox=None):
        ...
        # 解析HTML获取纯文本和对齐方式
        text, alignment = self._parse_html_text(content)
        
        if not text:
            return

        font_size, is_bold, _ = self._infer_text_style(text, bbox)
        is_heading = font_size >= self.config["heading_font_size"] or text.startswith('##')
...
def main():
    if len(sys.argv) < 3:
        print("用法: python step3_build_word_v2.py <layout_details_final.json> <output.docx>")
        sys.exit(1)
    ...
    converter.convert(layout_details, output_path)

    pages = len(layout_details)
    elements = sum(len(page) for page in layout_details)
    print(f"处理完成: {pages} 页, {elements} 个元素")
