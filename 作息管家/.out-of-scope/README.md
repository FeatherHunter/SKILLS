...
- **写入位置**：本目录下 `<slug>.md`，slug 取 issue 标题去除 `[作息管家]` 前缀后的归一化形式（小写、空格转 `-`、去除标点）。
- **必填 frontmatter**：
...
`.scratch/<feature>/issues/*.md` 视为历史归档（只读），不需要回填到本目录。新工作走 GitHub Issues，本目录同步新建。

## 与 GitHub issue 的关系

每次写入都必须在对应 GitHub issue 评论里留链接，例如：

> 已记录拒绝理由到 `作息管家/.out-of-scope/<slug>.md`，未来同类请求将引用此条。