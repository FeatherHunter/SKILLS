...
## 对抗式审查处置（2026-08-09 · 人类全部确认 · 详见 `.scratch/skill-levelup/issues/审查处置-决议.md`）

- **矛盾 1** git 历史 vs 可开源 → 人类接受（不洗历史、不 README 说明，风险显式接受）
- **矛盾 2** 合并器 → 新建 `scripts/update_scenarios.py` 正式合并器（替代 .notes 一次性脚本）
- **矛盾 3** 复盘本周/本月 → start-end 增强（预置粒度，升级 schedule_replay.html 呈现，复用路由）
- **矛盾 4** 周视图 → 复用 record_category 热力图组件 + 扩展全分类总览
- **模板分合规则**：一个场景=一个唤醒词=一个模板；呈现数据不同→独立模板；复用只在计算层；自由区间按长度路由（~7天→周模板 / ~30天→月模板 / 其他→通用）
...
## 规格 map 已关闭（2026-08-10）

规格决策全部在实施阶段兑现（实施 map #224 同关闭）。本文件作为规格阶段存档。
