...
### 🚀 T8 · L 级重构收尾 · 场景资产全量合并 + 文档 + 镜像(依赖全部 T0-T7)

**动机**:L 级重构实施 map 收尾票 —— 各票场景片段统一合并、文档补齐、HELP 镜像同步。

**变更**:
- ✅ **场景资产 73→85**:合并器全量拼接 6 片段(record 更新 5 / replay 更新 4 / plan +5 / batch +1 / setup +1 / week +1),无重复、幂等 byte-identical;「记录」= 合并 #0 记作息 5 子场景(三件套结果)/「制定计划」强化 #17(plan-result)/「批量导入」收编 #0-json(batch-add)/「复盘区间」= 原 start-end 更名;周视图场景注册归 T8(G2-A6,T6 不碰 scenarios.yaml)
- ✅ **README.md 新建**:30 秒开始三段式(复制即装)+ 功能速览 + 数据隐私 + 文件清单
- ✅ **SKILL.md**:首次使用章节(R3 6 步 + 飞书强引导)+ 模板分合规则(5 条)+ 场景注册替换关系 + 路由表补 L 级重构新增能力(批量导入/周视图/制定计划/首次使用)+ 记录链路旧称修正(add 自动渲染三件套 / render-record-result)
- ✅ **help 镜像**(ADR-0001):34 唤醒词 / 85 场景 / 1 待开发;CATEGORY_MAP 补 周视图(查询与浏览)/ 首次使用(辅助与管理),零未分类
- ✅ **修复**:help_render.escape_for_js 二次转义 bug(场景文本含引号 → HELP 页 JS 语法错误,round-trip 回归测试)
- ⚠️ **偏离记录**:G2 定标 81 vs 实测 85(复盘 start-end Phase E 从未注册=净新增+1 / 制定计划 5 变体 vs 计数 1=+4),人类确认口径后落库
