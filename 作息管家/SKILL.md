...
description: >
  作息时间记录与计划管理技能。当用户使用以下指令时触发:
   ...
   改计划、删计划、补计划、复盘、作息管家同步(新版 6 个命令,含 Phase 0 反向对账)、
  初始化数据库(管理类)、
...
| 2 | 同步作息(废弃) | 完整同步流程(获取→分析→写入) | `prepare-messages` → AI → `add_record_full` | 详见 1. 同步作息 |
...
| **20** | **作息管家同步** | Phase 0 反向对账 + diff 询问 create/update/delete | `feishu-resync <日期>` | **详见 8. 作息管家同步** |
