...
| **7** | **查作息详情** | **文本版**`detail [日期]` 含消息原文与人工智能推理链。**网页版**(2026-07-23 新增)`render-records-detail <日期> [--record-id N] [--show-source]` 聚焦活动摘要;默认不含消息原文与推理链,加 `--show-source` 展开;需回 AI 对话请求 | `detail [日期]` 或 `render-records-detail <日期> [--record-id N] [--show-source]` | **详见 3. 查询作息(详情页子章节)** |
