...
description: >
  ...
  HELP 命中后 invoke `scripts/help_render.py`,输出 `$SKILLS_DB_PATH/schedule_html/help/help_center.html`(总纲 §04 原则 9 · 命名不带版本 · 覆盖写)。
...
## 强制规定(与 SKILL开发总纲V1.0 同源)

1. **HTML 手册同步** - 唤醒词增删 / CLI 命令变化 / 用法调整时,必须同步更新 `作息管家.html` 手册页(确保用户看到的用法与代码一致)。本规定与 `SKILL开发总纲V1.0` 钩子 #1 "HTML 同步硬规则"同源,是总纲契约在本 Skill 的具体落地。
   - **本规定定位**:`作息管家.html` 是**用户面向的手册**(唤醒词 + 使用方法 + 展开按钮看底层工作流程),**不是改动日志**(改动见 `CHANGELOG.md`),**也不是 SKILL.md 的镜像**。
   - **本规定不要求**:任何代码改动都要同步 HTML(例如修 bug 调内部实现、改测试、加 fixture 都不需要动 HTML)。
   - **本规定要求**:用户能看到的接口面(唤醒词 / CLI 子命令 / 参数 / 行为)变化时,必须同步 HTML 手册(同 commit)。

> **HTML 推送策略**(2026-07-25 重构 · Phase A):跑完 `render-*` 命令后,默认用 `<media>` 内嵌 HTML 让用户在 IDE 看到;浏览器自动打开遵循用户偏好(详见 §"工程实践 · 浏览器自动打开 HTML 报告"章节的 `webbrowser` 跨平台方案),不再硬编码 Chrome 路径,不再强制 auto-open。

> **本规定的删除历史**(2026-07-25 · Phase A):
> - ❌ 删"此规定优先级最高"(违反总纲元规范)
> - ❌ 删"任何修改需用户确认"(属用户协作偏好,非契约)
> - ❌ 删"渲染 HTML 必须主动推送"全文(误读总纲 §04 原则 11,与 §"工程实践"中"拒绝的反模式(硬编码 Chrome)"自相矛盾)

---


> **版本**: v1.1.3 (2026-07-25) · Phase A:删自创"强制规定 #2/#3/#4",仅保留与总纲同源的 #1 + 推送策略文档化
...
| **7** | **查作息详情** | **文本版**`detail [日期]` 含消息原文与人工智能推理链。**网页版**(2026-07-23 新增,2026-07-23 修订 100% 字段暴露)`render-records-detail <日期> [--record-id N]` 每条记录全 11 字段都注入 payload,上层模板自主决定渲染样式 | `detail <日期>` 或 `render-records-detail <日期> [--record-id N]` | **详见 3. 查询作息(详情页子章节)** |
...
## 术语与唤醒词隔离 · 域边界声明(2026-07-25 重构 · Phase D.1)

作息管家是**宽 Skill**,内部管理两个语义不同的"域":

| 域 | 表 | 性质 | 触发词根 |
|---|---|---|---|
| **作息记录(record)** | `schedule_records` + `daily_summary` | 真实发生的过去/当下时间块(回顾性输入) | 作息 / 记录 / 流水 / 实际 / 同步 / 汇总 / 时间轴 / 对比 / 异常 / 类别 |
| **日程计划(plan)** | `schedule_plans` | 未来或当天的安排(预测性输入) | 计划 / 日程 / 安排 / 规划 / 商量 / 补 / 改 / 删 / 复盘 |

**强制规则**(2026-07-25 重申):
- 日程计划的唤醒词**不能**包含"作息/记录";作息记录的唤醒词**不能**包含"计划/日程"。
- 旧交叉词仅作为历史资料理解,不再写入唤醒词表。
- 跨域操作(如"商量计划"会涉及 record 上下文)走"跨域工作流"段。
...
- 写入:#0 记作息 / 同步作息(#1 #2 #3)
- 查询(文本):#11 查作息状态
- 查询(HTML):#4 今天总结 / #5 汇总作息 / #6 查作息 / #7 查作息详情 / #8 查作息时间轴 / #9 查作息范围
- 修正:#26 修正作息
- 派生:#23 按 ID 查记录 / #24 写作息摘要
- 深度分析:#25 对比两个月 / T4 类别深挖 / T5 异常检测

**日程计划域(11 个)**:
- 查询:#12 查日程 / #15 24h 概览 / #16 查多日计划
- 写入(主入口):#17 商量计划(多轮对话 → upsert)
- 写入(单条):#13 补计划
- 修改:#18 改计划
- 删除:#19 删计划
- 复盘:#14 复盘(completion 闭环)
- 飞书联动:#20 日程管家同步 / #21 飞书探测

**管理域(2 个)**:#22 初始化数据库 / Cron 类(已删)

### 跨域工作流(端到端闭环)

```
record 域(#0 记作息 / #1-3 同步作息)  ─── 写入真实数据
                                        ↓
plan 域(#17 商量计划)                  ─── 未来安排
                                        ↓
plan 域(#14 复盘)                      ─── completion 闭环到 record
                                        ↓
record 域(#26 修正作息)                ─── 数据修正
```

**第一性**:`商量计划`(plan 域)是规划未来,`复盘`(plan 域)是回顾当日 plan 执行情况,**两者闭环形成"事前规划 → 事中执行 → 事后回顾"完整循环**,不应拆分为 2 个 Skill(否则破坏跨域流程)。

### 跨 Skill 路由声明(总纲 §03 铁律 3 · 2026-07-25 新增)
...
IF 唤醒词是 "作息管家 HELP" 类(§07 契约)
   THEN invoke help_render.py(输出 `$SKILLS_DB_PATH/schedule_html/help/help_center.html`)
ELIF "输出形式"列含 .html
...
|---|---|---|---|
| "作息管家 HELP / 帮助 / 能做什么 / 使用说明" | (HELP 唤醒词,§07 契约) | `python scripts/help_render.py` | `$SKILLS_DB_PATH/schedule_html/help/help_center.html` |
