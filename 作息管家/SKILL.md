...
1. **HTML 手册同步** - 唤醒词增删 / CLI 命令变化 / 用法调整时,必须同步更新 `作息管家.html` 手册页(确保用户看到的用法与代码一致)。
   ...
   - **本规定要求**:用户能看到的接口面(唤醒词 / CLI 子命令 / 参数 / 行为)变化时,必须同步 HTML 手册。
2. **此规定优先级最高** - 优先级高于所有其他规则和流程。
3. **任何修改需用户确认** - 对本技能所有文件、脚本的任何一行修改,都必须明确得到用户的 1 次确认后才能执行。
4. **渲染 HTML 必须主动推送** (2026-07-25 新增 · 用户报告触发) - 跑完任何 `render-*` 命令后:
   - **必须**主动做这 3 件事(顺序固定):
     1. 用 AI 工具的 `<media src="<file_path>" type="file" />` 把生成的 HTML 内嵌/发送给用户(用户能在聊天窗口直接看)
     2. 用操作系统默认浏览器自动打开这个 HTML(用户双屏工作流:A 屏 IDE 看对话,B 屏浏览器自动开)
     3. 用 1-2 句话**文字确认**给用户("已生成 X 报告 + 已自动打开" + 附路径供后续参考)
   - **禁止行为**:
     - 只输出 CLI 的 JSON 给用户就停 — 让用户必须再说"生成一下"才被动给(本规则的直接触发原因)
     - 只输出 `<media>` 不打开浏览器 — 用户需手动操作
     - 只打开浏览器不输出 `<media>` — 用户在聊天窗口看不到
     - 在用户没明确反对前询问"要不要我打开浏览器?" — 默认就是主动
   - **例外**(可不必主动推):
     - 用户明确说"只输出路径"/"不要自动开"/"我来手动开"
     - 用户在 `safe_render` 模式(未来功能):只渲染不推送
     - 同一次对话内 5 分钟内已推过同一文件(避免重复打扰)
   - **违反本规定的标志**(自查):
     - Agent 跑完 render 后输出 "✓ 已写入: /path/to/file.html" 后停 → **违规**
     - Agent 输出文件路径但没说"已打开" → **违规**
     - Agent 说"需要的话我可以打开" → **违规**(直接自动打开)

**为什么需要这条强制规定**:
- 用户体验:**默认推送** > 询问"要不要推送" > 等待用户主动要求
- 一致性:**所有 render-* 命令 100% 推送**,不挑命令
- 与"工程实践"7 步流程 Step 5/6/7 升级:从"建议"变"强制"
