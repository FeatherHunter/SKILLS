...
| 用户场景 | 用哪个 |
|---|---|
| "查计划" / "看计划" / "今天的安排"（**默认**） | `list-events <日期>` |
...
> 备注:本节合并了原"7. 看计划"小节,语义统一在"查询计划"下。
...
- **B 阶段新增失败处理**:
  ...
  - 心愿排期失败（备忘录 set-due 返回 error） → DB 写入已成功，飞书侧 due 可能没同步；提示用户跑 `备忘录同步飞书` 或 `心愿排期` 重试
...
> 原"看计划"小节已合并到 `3.1 查询计划`,触发词"看计划 / 列出今日计划 / 看一天的计划"统一指向 `list-events`。
