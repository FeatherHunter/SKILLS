...
def refine_category(rec):
    ...
    real_dur = calc_duration(rec['time_start'], rec['time_end'])
    
    # 0 时长跳过
    if real_dur == 0:
        return orig_cat  # 保留原分类
    
    # 「休闲」类不做修改，但细分睡眠 vs 赖床
    # 「睡眠」类如果实际是躺床没睡着，归休闲
    if orig_cat == '睡眠':
        if any(kw in activity for kw in ['躺床休息', '床上赖着', '凌晨床上休息', '累', '赖床']):
            return '休闲'
...
for r in records:
    new_rec = {**r}
    new_rec['category'] = refine_category(r)
...
print(f"细化后总计: {total_check} min = {total_check//60}h{total_check%60}m")
print("\n各分类汇总:")
...
print(f"📌 主睡眠: {main_sleep['time_start']}~{main_sleep['time_end']} ({main_sleep['real_duration']}min)" if main_sleep else "📌 主睡眠: 无")
...
print(f"📌 学习段数: {len(cat_records.get('学习', []))}")
print(f"📌 娱乐段数: {len(cat_records.get('娱乐', []))}")
print(f"📌 兴趣爱好段数: {len(cat_records.get('兴趣爱好', []))}")