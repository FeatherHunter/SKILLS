...
def calc_duration(ts, te):
    ...
    # 处理跨日: time_end < time_start (如 23:20~00:22)
    if eh * 60 + em < sh * 60 + sm:
        return (24 * 60 + eh * 60 + em) - (sh * 60 + sm)
...
if records:
    ...
    if first_start == '00:00':
        ...
        print(f"⚠️ 首条 {first_start} 不是 00:00 (本数据 00:22 开始,前22min属7/1)")

    if last_end in ('23:59', '00:22'):
        print(f"✓ 末条 {last_end} 覆盖到跨日 00:22 (24h 闭环)")
    else:
        print(f"⚠️ 末条 {last_end}")
    ...
    # 7/2 范围: 00:22 ~ 次日 00:22 = 24h
    if not gaps and total_real == 1440:
        print("\n✅ 满足满24小时条件（首尾相接、跨日 24h 闭环）")
        coverage_ok = True
    else:
        print(f"\n❌ 不满足满24小时条件 (real={total_real}, expected 1440)")
        coverage_ok = False
else:
    print("❌ 无记录")
    coverage_ok = False
...
print()
if total_check == 1440:
    print("✓ 总计 1440 min 正好覆盖 24h")
else:
    print(f"⚠️ 总计 {total_check} min != 1440 min")
...
for r in records_refined:
    try:
        ...
        print(f"⚠️ {r['time_start']}~{r['time_end']}: {e}")
        pass
...
hobby_records = [r for r in records_refined if r['category'] == '兴趣爱好']
home_records = [r for r in records_refined if r['category'] == '家务']
social_records = [r for r in records_refined if r['category'] == '社交']
wash_records = [r for r in records_refined if r['category'] == '洗漱']
exercise_records = [r for r in records_refined if r['category'] == '运动']
rest_records = [r for r in records_refined if r['category'] == '休息']
qijv_records = [r for r in records_refined if r['category'] == '起居']
plan_records = [r for r in records_refined if r['category'] == '计划']
...
print(f"📌 洗漱段数: {len(wash_records)}")
print(f"📌 兴趣爱好段数: {len(hobby_records)}")
print(f"📌 社交段数: {len(social_records)}")
print(f"📌 家务段数: {len(home_records)}")
print(f"📌 起居段数: {len(qijv_records)}")
print(f"📌 计划段数: {len(plan_records)}")
print(f"📌 休息段数: {len(rest_records)}")
print(f"📌 运动段数: {len(exercise_records)}")
