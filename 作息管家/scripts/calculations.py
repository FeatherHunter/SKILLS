...
# M5 别名映射(2026-07-23):老一级词 → 新一级词
# 背景:旧 cat "运动·跑步" l1="运动" 但 8 L1 表("维持/健康/工作/学习/创作/投入/调整/日常")不含"运动"
# 渲染时归"未知" → 用户查"运动"拿到 0 分钟报告(误以为没数据)
# 修法:老的"运动"/"通勤"/"休息"/"起居"等映射到新分类的"健康"/"维持"/"调整"
# 不映射的不动(罕见老词)
_L1_ALIAS = {
    "运动":   "健康",   # 旧"运动"=新"健康"(健身/运动/修行 全部归"健康")
    "通勤":   "维持",   # 旧"通勤"=新"维持.通勤"
    "休息":   "调整",   # 旧"休息"=新"调整.休息"
    "起居":   "维持",   # 旧"起居"=新"维持.洗漱"
    "娱乐":   "调整",   # 旧"娱乐"=新"调整.游戏"
...
def l1_of(category: str) -> str:
    ...
    M5:对老一级词(运动/通勤/休息/起居 等)做别名映射到新 8 L1
    """
    if not category:
        return "未知"
    for sep in (".", "·", "・", "•"):
        if sep in category:
            base = category.split(sep)[0]
            return _L1_ALIAS.get(base, base)  # 命中别名映射,否则原样
    # 旧 cat 无分隔符,直接是单级词 → 别名映射
    return _L1_ALIAS.get(category, category)
