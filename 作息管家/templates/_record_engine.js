...
(function(){
  ...
  // record 域 6 模板共享:委托 window.CopyPromptHelper(_copy_prompt_helper.js)
  // 该 helper 也被 schedule_list_events.html 等其他模板引用,逻辑只有一份。
  function copyPromptBlock(data){
    if (window.CopyPromptHelper) {
  ...
  // 注:点击事件由 CopyPromptHelper.bind() 全局代理(.copy-prompt-btn class),
  // 此处不再重复注册 document.addEventListener("click", ...)
})();
