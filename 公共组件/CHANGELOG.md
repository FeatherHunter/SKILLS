...
- **① 术语「复制 prompt」→「复制指令」**: hero 步骤条（📋 复制指令）+ 空值拦截 toast（「填写完整后再复制指令」）+ **formPrompt 参数表单按钮**（base.js 2 处: 初始文案 + 实时预览重置文案）; 08 规范 HELP 流程术语同步修订（SKILL开发总纲/08-HTML交互规范.md 3 处: 导航流程/确认变更/转移清单）+ README §4.4 验收文案同步
...
- **描边**: 1.5px 主色实边 → `1px + color-mix(主色 35% 透明度)`（不支持 color-mix 的旧浏览器回退主色实边）
