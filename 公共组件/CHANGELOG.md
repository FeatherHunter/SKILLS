...
## v1.30（2026-08-14 · smooth 曲线过点修复 · #381 几何采样发现）

**charts.line smooth 曲线不经过中间数据点修复**（非破坏性 · 纯内部实现 · 签名零变更；用户对抗式审查（几何采样）发现，遗留清单项 2）。

- **现象**：`smooth:true` 时文档/注释声明「Catmull-Rom，点在线」，但实现上中间数据点不在曲线上（数据点"飘"在曲线外，曲线峰值塌陷 ~26 SVG 单位）
- **根因**：`_smoothPath` for 循环 off-by-one（`for(i=1;i<s.length-1;i++)`），每段 C 曲线从 `s[i-1]` 出发、终点 `s[i+1]`，跳过 `s[i]` 作为曲线节点
- **修复**：改为标准均匀 Catmull-Rom→Bezier 转换（每段 `P_i → P_{i+1}` 以 `P_i` 为起点，控制点基于 `(P_{i-1}, P_i, P_{i+1}, P_{i+2})`，首尾段控制点端点钳制 phantom point extension——业界惯例让首尾点处一阶导数 = 邻段斜率 C¹ 连续）。曲线严格经过全部数据点（合约 §6.5「点在线」兑现）
- **附带**：守卫测试 +5（曲线过点几何断言 / [5,15,5] 最小复现 / smooth+series 多序列 on-curve / smooth+connectNulls 几何补强 / smooth 缺省 ≡ false 字节回归）。沿用 #379 教训：图表守卫测试必须含渲染几何断言（getPointAtLength 采样 / getBoundingClientRect），style 字符串断言抓不到视觉缺陷
- 契约 §6.5 smooth 条目 + §0 版本记录 + CHANGELOG 同步；守卫测试 +5 → 全量全绿（tests/ 295/295）

## v1.29（2026-08-14 · stacked 段高度塌陷修复 · #379 验收发现）

**charts.bar stacked 堆叠柱段高度塌陷修复**（非破坏性 · 纯样式 · 签名零变更；用户双端验收发现）。

- **现象**: `stacked:true` 时每段柱子只有几毫米高（百分比高度相对 auto 父容器塌陷）
- **根因**: v1.23 引入的 `.hm-c-stack` 缺 `height:100%`/`flex:1`——段 `height:30%` 等百分比相对 auto 高度父容器塌陷为 min-height 2px；守卫测试只断言 style 字符串（`n.style.height == "30%"`）未断言实际渲染高度，v1.28 双轴对抗审查同样漏过（审查盲区）
- **修复**: `.hm-c-stack` 改 `flex:1`（撑满柱列 label 之外剩余空间）; 守卫测试补「实际渲染高度」断言（stack 高 ≥ 柱列-25px / 段高 > 5px / 最大段 > 30px）
- **附带**: charts.js 头注释 v1.25 → v1.29 补跳（v1.26-1.28 改代码未升头注释, 一致性漏点一并修正）
- 契约 §0 v1.29 行 + CHANGELOG 同步; 守卫测试 +1 → 全量全绿（tests/ 290/290）

## v1.28（2026-08-14 · 对抗式审查修复包）

**双轴对抗审查（Standards/Spec 子代理并行 + 编排者独立复核）后的收口修复**（非破坏性 · 一致性/缺陷修补 · 不触碰已拍板语义）。

- **契约 §0 补录 v1.25-1.27 三行**（审查发现漏记，标题由 v1.25 升 v1.28; 对齐 v1.13 补录先例）
- **§6.5 全参数表 line/bar 行补全新参数**（band/fillBetween/highlightPoints/anomaly/showValues-edge/markLine-xValue/markPoint 版本标注 + bar stacked/grouped/segNames/stackMode）
- **fillBetween 补 connectNulls 语义**: 不随 connectNulls 跨空（仿 band 口径, 消除审查发现的未定义交互）
- **scatter `dotSize` 声明即实现**: 原为死参数（声明未使用）——按 #319 先例「契约声明必须实现」改为实现（数值=直径 px, 内联尺寸+居中 margin, 缺省 9px）
- **grouped 尊重 yMin/yMax**: 对齐单柱语义（此前忽略, 契约表述与实际不符）; stacked 按契约不参与不变
- **ghost 透明度 35%→38%**: 与拍板页（329-ghost对比拍板页.html B 方案 rgba .38）所见一致
- **08 规范 L134-135 追加修订注记**: 保留 2026-08-10 裁定 + 标注 2026-08-14 #329 方案① 修订基准（描边 1px/38%、12px/600、40px）
- **活跃文档术语残留统一**: 「复制 prompt」→「复制指令」——总纲 04/07/09/09.1/CONTEXT/SKILL.md + 公共组件 help-template-contract/scene-data-contract/scene_data.schema.json（历史归档 reviews/*.html 与 html 导出物不动）
- **守卫测试 +3**（markPoint 最右贴边 / scatter dotSize 生效 / grouped yMin-yMax）→ 全量全绿

## v1.27（2026-08-14 · HELP 模板用户语言修正 · #330 用户拍板采纳 ①+④）

**help_template.html 文案用户语言化**（非破坏性 · 纯文案/模板 · 签名零变更；用户 2026-08-14 拍板采纳第 1、4 项，第 2（徽章）、3（Tab）项未采纳保持现状）。

- **① 术语「复制 prompt」→「复制指令」**: hero 步骤条（📋 复制指令）+ 空值拦截 toast（「填写完整后再复制指令」）+ **formPrompt 参数表单按钮**（base.js 2 处: 初始文案 + 实时预览重置文案）; 08 规范 HELP 流程术语同步修订（SKILL开发总纲/08-HTML交互规范.md 4 处: 导航流程/转移清单/确认变更×2）+ README §4.4 验收文案同步
- **④ 顶部 lead 用户视角**: 「N 场景 · 点卡片看详情」→「N 场景 · 点击卡片查看详情并复制指令」
- **未采纳项保持现状**: ②类型徽章多色、③Tab 选中态大蓝色块——按用户拍板不动
- 守卫测试 +3（渲染输出含「复制指令」/新 lead/无「复制 prompt」残留）→ 全量全绿

## v1.26（2026-08-14 · 复制按钮 ghost 样式降权重 · #329）

**button.copy.ghost 视觉降权重**（非破坏性 · 纯样式 · 签名零变更；用户 2026-08-14 拍板方案① 降权重不动布局）。

- **描边**: 1.5px 主色实边 → `1px + color-mix(主色 38% 透明度)`（不支持 color-mix 的旧浏览器回退主色实边；v1.28 由 35% 对齐拍板页）
- **字号/字重**: 13.5px/700 → `12px/600`; **高度**: min-height 48px → `40px`
- **布局零变更**: 复制数据/日志仍独立一行（08 规范硬标准不动, 方案① 不触碰规范）; 主操作（primary 48px 蓝实心）视觉重心恢复
- **全技能生效**: base.css 单点修改; 契约 §6.2 + README §4 同步; 守卫测试 +1（ghost 计算样式 40px/12px/600/1px 浅描边）→ 全量全绿
