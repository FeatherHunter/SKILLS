...
4. **验收**: 与 V4.16 原型视觉一致（用户验收）+ 复制指令 正确（参数表单/空值拦截/toast）+ 手机三档视口 0 溢出
...
**按钮规范**（对齐 08 规范）: ≤3 色按功能区分（蓝=主操作 / 红=危险 / ghost=复制）;偶数一行 2 个, 奇数重要按钮加 `.wide` 单独一行;复制数据/日志 = 白底+浅主色描边(38% 透明度)+主色文字胶囊, min-height 40px、12px 600（v1.26 · #329 降权重: 视觉重心归主操作; v1.28 对齐拍板页）。
