...
> 来源：v1.3（#288 图表组件落地）+ v1.4（#290 状态层加固）+ v1.5（#289 HELP 参数化）+ **v1.6 图表全参数化**（2026-08-12：方向 A 定稿，四形态全参数 + 复合形态）+ **v1.9 selectList 行内控件**（2026-08-13 · #327）+ **v1.10 copyText 反馈钩子**（2026-08-13 · #328）+ **v1.11 smartSelect 选择器组件**（2026-08-13 · #312 · 立项 #320）+ **v1.12 smartSelect 候选区折叠**（2026-08-13 · #312 实测反馈）+ **v1.13 line 动画断线修复**（2026-08-14 · #317 验收二轮）+ **v1.14 charts.line connectNulls**（2026-08-14 · #356）+ **v1.15 charts.line yTicks 轴刻度**（2026-08-14 · #333）+ **v1.16 markLine 标签拉伸/遮挡修复**（2026-08-14 · #333 验收）+ **v1.17 series[].ownScale 系列独立刻度**（2026-08-14 · #334）+ **v1.18 markPoint 峰谷点标注**（2026-08-14 · #319）+ **v1.19 band 置信带**（2026-08-14 · #335）+ **v1.20 markLine 竖线 xValue**（2026-08-14 · #340）+ **v1.21 fillBetween 线间填充**（2026-08-14 · #338）+ **v1.22 小缺口：异常点/拐点圈选/首尾标签避让**（2026-08-14 · #341）+ **v1.23 bar 堆叠柱 stacked**（2026-08-14 · #336）+ **v1.24 bar 分组双柱 grouped**（2026-08-14 · #339）+ **v1.25 charts.scatter 散点图**（2026-08-14 · #337）+ **v1.26 ghost 降权重**（2026-08-14 · #329）+ **v1.27 HELP 术语**（2026-08-14 · #330）+ **v1.28 审查修复包**（2026-08-14 · 对抗式审查收口）+ **v1.29 stacked 高度塌陷修复**（2026-08-14 · #379 验收发现）+ **v1.30 smooth 曲线过点修复**（2026-08-14 · #381 几何采样发现）
...
| v1.25 | 2026-08-14 | **#337 新增 charts.scatter 散点图**（非破坏性 · 新增接口 · 既有接口零变更）：`charts.scatter(el, items, opt)`——items `[{x, y, label?}]` 双数值坐标（非法 x/y 直接报错，空数组 → emptyState）；线性回归线（最小二乘，`regression:false` 关闭，`regressionColor` 缺省 #ff3b30 虚线）；Y 轴刻度复用 line 的 yTicks 机制（scatter 缺省 4 条，读轴是散点核心用途）；X 轴标签 edge/all/none；format/tooltip（最近点命中）/animation（点淡入）/双端自适应对齐 line。守卫测试 +6 → 全量全绿（tests/ 285/285）。详见 §6.5（本行为 §0 漏记补录，CHANGELOG 已有 v1.25） |
| v1.26 | 2026-08-14 | **#329 复制按钮 ghost 样式降权重**（非破坏性 · 纯样式 · 签名零变更 · 用户拍板方案①）：描边 1.5px 主色实边 → 1px + color-mix(主色 38% 透明度)；字号 13.5px/700 → 12px/600；min-height 48px → 40px；布局/08 规范结构零变更。守卫测试 +1 → 全量全绿（tests/ 286/286）。详见 §6.2（本行为 §0 漏记补录） |
| v1.27 | 2026-08-14 | **#330 HELP 模板用户语言修正**（非破坏性 · 纯文案 · 签名零变更 · 用户拍板采纳①+④）：「复制 prompt」→「复制指令」（hero 步骤条/toast/formPrompt 按钮）+ 顶部 lead「点击卡片查看详情并复制指令」；08 规范术语同步 4 处 + README 同步；②徽章/③Tab 未采纳保持现状。守卫测试 +3 → 全量全绿（tests/ 286/286）。详见 §6.2（本行为 §0 漏记补录） |
| v1.28 | 2026-08-14 | **对抗式审查修复包**（非破坏性 · 一致性/缺陷修补）：①契约 §0 补录 v1.25-1.27（本批次对抗审查发现漏记）②§6.5 全参数表 line/bar 行补全新参数 ③fillBetween 补 connectNulls 语义（仿 band「不随 connectNulls 跨空」口径）④scatter `dotSize` 声明即实现（删除改为实现，防再造 #319 同类「声明未实现」）⑤grouped 尊重 yMin/yMax（对齐单柱语义）⑥ghost 透明度 35%→38%（与拍板页所见一致）⑦08 规范 L134-135 追加修订注记 ⑧活跃文档「复制 prompt」术语残留统一（总纲 6 文件 + 公共组件 3 文件）⑨markPoint 最右贴边守卫测试。守卫测试 +3 → 全量全绿（tests/ 289/289）。 |
| v1.29 | 2026-08-14 | **#379 charts.bar stacked 段高度塌陷修复**（非破坏性 · 纯样式 · 签名零变更 · 用户验收发现）：`.hm-c-stack` 缺 `flex:1` 导致段百分比高度相对 auto 父容器塌陷（几毫米）；修复 + 守卫测试补「实际渲染高度」断言（style 字符串断言盲区）；charts.js 头注释 v1.25→v1.29 补跳。守卫测试 +1 → 全量全绿（tests/ 290/290）。 |
| v1.30 | 2026-08-14 | **#381 charts.line smooth 曲线不经过内点修复**（非破坏性 · 纯内部实现 · 签名零变更 · 对抗式审查（几何采样）发现）：`_smoothPath` 循环 off-by-one 改为标准 Catmull-Rom→Bezier 转换（曲线严格经过全部数据点 · §6.5「点在线」兑现）。守卫测试 +5 → 全量全绿（tests/ 295/295）。 |
...
- 按钮规范（08 规范）：≤3 色按功能区分；ghost = 白底+浅主色描边（38% 透明度）+主色文字胶囊、min-height 40px、12px 600、独立一行（v1.26 · #329 降权重：视觉重心归主操作；v1.28 透明度对齐拍板页）；偶数一行 2 个
...
- **scatter（v1.25 · #337）**：新增接口 `charts.scatter(el, items[, opt])`——items: `[{x, y, label?}]`（x/y 双数值坐标，非法值直接报错；空数组 → emptyState）；**回归线**：线性最小二乘（n≥2 时渲染，`regression: false` 关闭，`regressionColor` 缺省 #ff3b30 虚线）；**Y 轴刻度**：复用 line 的 yTicks 机制（数字 2-6 / false 关闭；**scatter 缺省 4 条**——读轴是散点核心用途，与 line 默认关闭不同）；**X 轴标签**：`labels: 'edge'`（缺省，首尾）/ `'all'` / `'none'`；`format` 同其他接口（刻度/tooltip 同一格式化器）；`tooltip: true` 最近点命中（显示 label + x · y）；`animation` 点淡入；`height`/`color`/`dotSize` 可选（dotSize 数值=散点直径 px，内联覆盖居中，缺省 9px · v1.28 实现）；双端自适应沿用 line 语义（≤720px 点 8px）；坐标唯一性同 line（容器零 padding，viewBox 留白）
...
- **fillBetween（v1.21 · #338）**：`line` 可选参数，默认 `null`（行为与 v1.20 逐字节一致）；`fillBetween: {a, b, color?}`——a/b = `series[]` 索引（数字，越界/相同/非数字/两系列 items 长度不一致 → 直接报错），两线之间区域半透明填充（透明度 = `areaOpacity`，绘制在折线路径之前不遮挡；`color` 覆盖填充色，缺省主色）；任一序列该点 `null` → 该段断开不跨空填充；**不随 connectNulls 跨空**（与 band 口径一致，v1.28 审查修复明确）；一次一组填充（不支持多组叠加、不做上/下侧语义）；与 band/markLine/series 正交
...
| `bar` | `format` · `colors/singleColor` · `height/compact` · `labels('all'/'none'/'select')` · `showValues` · `yMin/yMax/grid` · `tooltip/animation` · `onclick` · `stacked`（堆叠柱 · v1.23）· `grouped`（分组双柱 · v1.24）· `segNames`（段名图例/tooltip）· `stackMode:'percent'|'absolute'`（缺省 percent） |
