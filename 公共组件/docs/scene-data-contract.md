...
一个场景可挂**多个类型标签**（如「过程 + 结果」：先发表单 HTML 给用户填 → 复制回 AI → 执行后回执）。两种写法：

```json
"types": ["采集"]                                   // 字符串: 默认配色
"types": ["过程", "结果"]                            // 多标签: 各自默认配色
"types": [                                          // 自定义颜色（bg 背景 / fg 文字, 缺省走默认）
  { "text": "过程", "bg": "#e2f7f5", "fg": "#00897b" },
  "结果"
]
```

- 默认配色表（内置）：采集/录入=绿、查看/结果/回执/校验/选择=蓝、向导/过程=青、批量=紫；未知名 = 蓝兜底
- 卡片空间：1~2 个徽章最稳（手机 390px）；3 个以上会自动换行，**建议折叠进详情弹层（Sheet）展示**
...
|---|---|---|---|
| `id` | string | ✅ | 块唯一标识 |
| `title` | string | ✅ | 块标题（透传字段 · 页面不渲染展示） |
| `html` | string | ✅ | **技能方提供的 HTML 原文**，Base 原样透传字段（页面不渲染；转义需求技能方自理；禁止 `</script>`/`</style>` 字样混入资产注释） |

Base 当前不渲染展示 meta_blocks（对齐 V4.16 定稿原型——无此区块）；技能重构票如需展示，由技能侧自行扩展渲染。
