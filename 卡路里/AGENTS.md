...
> **当前处于 v1.0 场景重设计阶段。`_triggers.py` 旧版 80 个 wake_word 已部分替换:目标管理 25 / 基础信息 4 / 身体细节 13 / 运动 39 / 体重 58 已同步,其余分类仍为旧版运行时数据,不代表最终设计。**
...
- 已落盘 JSON 268(主页 9 + 目标管理 25 + 基础信息 4 + 身体细节 13 + 分析 154 + 健身计划 29 实施中 + 运动 39 + 体重 58;其余分类由实施 session 填)
- **已同步 `_triggers.py`**(432 词):目标管理 25(ticket #7)+ 基础信息 4(ticket #8)+ 身体细节 13(ticket #9 · 2026-08-02)+ 运动 39(ticket #5 · 2026-08-02)+ 分析 154(ticket #11 · 2026-08-02)+ **主页 9(ticket #2 · 2026-08-02,替换旧 1 词,aliases 开卡路里/卡路里面板/今日卡路里 合并保留)**
- **已定稿存档 `docs/scene-prompts/`**:01-主页(9)/02-饮食(68)/03-体重(58)/04-运动(39)/05-健身计划(29)/06-目标管理(25)/07-基础信息(4)/08-身体细节(13)/09-身材照片(10)/10-分析(154)——11 分类剩 11-技能协同(37)待开发
- **主页 9 场景落地要点(2026-08-02 · ticket #2)**:6 KPI 卡口径(饮食/运动/体重/目标/进度/连续,4 KPI→6 KPI 为对抗审查修复 `d685130`)+ 7 天趋势小图 + 一句话总结;render_home.py 支持 `--section {diet,exercise,weight,goals,streak,budget}` / `--period {week,month}` / `--chain` 强制(R3);HELP 432 场景
