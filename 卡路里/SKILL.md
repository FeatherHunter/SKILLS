...
| `templates/profile_setup.html` | 设置档案 | `profile.get/set` | `scripts/render_profile_setup.py [--live]` |
...
| `templates/crud_receipt.html` | 删吃的 / 改吃的 / 改食品 / 存食品 / 改体重记录 / 改运动记录 / 改照片标签 / 删身材照 / 设活动量 / 改档案 | 各 CRUD 函数返回 diff | `scripts/render_crud_receipt.py` |
...
| 查营养结构 | 营养素占比分析 | `AI 路由(Python API)` |
...
| 唤醒词 | 功能 | CLI |
|--------|------|-----|
| 设置档案 | 填 4 项(身高/年龄/性别/活动量);**采访式引导 = AI 默认交互**:用户没说全时逐项问 + 按日常情况推荐活动量 | `python scripts/render_profile_setup.py --live`(接 DB)或 `calorie_tracker.py profile set <age> <gender> --height <cm> --activity <level>` |
...
###### 设置档案 / 设活动量 / 改档案(已实现 · render_profile_setup.py / calorie_tracker.py profile,配置型 + 回执型)

> **设置档案**:打开 HTML(--live 预填当前值,含活动量 5 档)或 AI 采访式引导逐项询问 → 生成 prompt → AI 执行 `calorie_tracker.py profile set <age> <gender> --height <cm> --activity <level>`
...
| `设置档案 30 male 177 --activity moderate` | AI 解析后直接调 CLI,无需 HTML |
...
| 查围度趋势 | 时间线 | `python scripts/body_measurements.py trend --metric waist-cm` → **HTML:`body_measurements_wizard.html`** |
...
python scripts/calorie_tracker.py profile activity <level>       # 只设活动量(sedentary/light/moderate/active/very_active)
...
- **设置档案**:`render_profile_setup.py --live`(HTML 预填当前值,改完复制 prompt)或 `profile set <age> <gender> --height <cm> --activity <level>`
...
2. 信息齐 → 调 `render_profile_setup.py --live` 打开配置页(预填当前值)或直接 `calorie_tracker.py profile set <age> <gender> --height <cm> --activity <level>`
3. 写库后 → 回执 HTML(呈现:身高/年龄/性别/活动量 + 推荐活动量 + 设置时间)
