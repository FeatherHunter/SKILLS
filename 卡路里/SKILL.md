...
| 看体脂 | 历史 + 来源筛选(皮褶钳/健身房/医院/全部)| `python scripts/body_composition.py list [--source <s>]` → **HTML:`body_composition_wizard.html`(list 视图)** |
| 看体脂趋势 | 折线(默认最近来源,可切换)| `python scripts/body_composition.py trend [--source <s>]` → **HTML:`body_composition_wizard.html`(trend 视图)** |
| 看围度 | 历史 + 部位筛选 | `python scripts/body_measurements.py list [--metric <col>]` → **HTML:`body_measurements_wizard.html`(list 视图)** |
| 看围度趋势 | 折线(先选部位)| `python scripts/body_measurements.py trend --metric <col>` → **HTML:`body_measurements_wizard.html`(trend 视图)** |
| 对比体脂 | 两段时间对比(注明同来源)| `python scripts/body_composition.py compare --start1 <D1> --end1 <D2> --start2 <D3> --end2 <D4> [--source <s>]` |
| 对比围度 | 两个日期 13 项 Δ | `python scripts/body_measurements.py compare --date1 <D1> --date2 <D2>` |
| 删体脂 | 软删除(先列候选 → 快照确认 → 回执)| `python scripts/body_composition.py delete --id <ID>` → **HTML:`crud_receipt.html`** |
| 删围度 | 软删除(先列候选 → 快照确认 → 回执)| `python scripts/body_measurements.py delete --id <ID>` → **HTML:`crud_receipt.html`** |
