...
cur = conn.cursor()
cur.execute('SELECT id, food_name, calories, protein, carbs, fat, grams, note, created_at FROM food_log WHERE date = "2026-07-11" ORDER BY created_at')
rows = cur.fetchall()
...
cur.execute('SELECT SUM(calories), SUM(protein), SUM(carbs), SUM(fat), COUNT(*) FROM food_log WHERE date = "2026-07-11"')
tot = cur.fetchall()
print("-" * 110)
