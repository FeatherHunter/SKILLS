...
def validate_scene(scene: dict, file_label: str, all_wake_words: dict[str, str]) -> list[str]:
    ...
    if data_src and not data_src.endswith('.html'):
        # 当 data_source 是 .py CLI 或 Python 模块路径,放宽(不强校验)
        if data_src.endswith('.py') or '.' in Path(data_src).name:
            # 如果是 .py 文件,校验存在
            if data_src.endswith('.py') and not (SKILL_DIR / data_src).exists():
                errors.append(f'[data_source] CLI 文件不存在: {data_src}')
