...
def test_kpi_grid_is_four_columns_on_desktop(tmp_path):
    ...
    with sync_playwright() as playwright:
        ...
        grid = page.evaluate("""() => {
          ...
          return { count: cards.length, widths };
        }""")
        browser.close()
...
def test_date_column_shrinks_on_iphone_xr(tmp_path):
    ...
    # 与 v2.5.20 (25%) 对比:调整后宽度应明显小于调整前 ~109px(实测 25%=>109px, 23%=>101px)。
    assert date_cell["headerWidth"] <= 102, date_cell
...
def test_date_column_does_not_regress_on_desktop(tmp_path):
    ...
    # 1280 桌面上日期列宽 ≈ 23% × (1280 - section padding);v2.5.20 25% 时代码为 25% × table_width。
    # 调整后不应回到 25% 的 222px。最差也是体重列 (kg) 0.2× table + 共享份额。
