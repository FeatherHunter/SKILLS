...
> **当前版本:1.1.1**(2026-07-25 发布 · 20 个 commit · git tag `v1.1.1`)
...
1. **必须用 `<media src="..." type="file" />` 标签交付 + 主动发送给用户**(基础动作 · v1.1.1 加强)。
   - 这是《预置HTML并注入数据指导手册》§4 + §9(标准工作流最后一步)规定的标准交付方式
   - HTML 内嵌 JS + clipboard + textarea,有交互,**内联展示会丢失交互**(所以内联禁止)
   - **v1.1.1 加强**:`<media>` 不够 —— 用户可能在手机/微信/飞书/QQ 等,**AI 必须主动把 HTML 文件发送出去**
     - 不能假设用户就在 Chrome 前看
