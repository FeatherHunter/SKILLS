...
- 命令：`script/memo_cli.py add "内容" [-c 顶层分类] [-s 子分类]`
- **顶层分类**（4 种）：备忘（默认）/ 心愿 / 打卡 / 情绪日记
- **子分类**（仅 category=备忘 时用，6 种）：社交 / 工作 / 学习 / 灵感 / 记账 / 成就
- **AI交互规范**：添加前必须先问用户一个问题确认分类（如下示例），不得直接使用默认分类写入
  ...
  - **子分类默认行为**：用户说"记备忘"但没说子分类时 → 写入 `备忘` + `sub_category=NULL`，AI 不必追问
...
- **子分类规则**：更新 sub_category 时要求 effective category（现有或新设）必须为"备忘"
...
- **副作用**：改顶层分类时会自动清空 `sub_category`（因为 sub_category 只对 category=备忘 有意义）

### 编辑笔记子分类
- 触发词：备忘改子分类
- 先搜索找到笔记 ID（必须是 category=备忘 的笔记），再更新子分类
- 命令：`script/memo_cli.py update-sub-category <id> <子分类 | null>`
- **规则**：只对 category=备忘 的 note 生效；传 `null` 表示清除子分类
