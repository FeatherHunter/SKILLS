...
**触发词**：记备忘、搜备忘、查备忘、改备忘、删备忘、看备忘、按时间搜备忘、备忘改分类、记提醒、设提醒、看提醒、查已提醒备忘

**分类子唤醒词**（心愿/打卡/情绪，自带分类，操作同上）：
- 记心愿、删心愿、改心愿、查心愿
- 记打卡、删打卡、改打卡、查打卡
- 记情绪、删情绪、改情绪、查情绪
...
- **AI交互规范**：添加前必须先问用户一个问题确认分类（如下示例），不得直接使用默认分类写入
  ...
  - 示例4：用户已明确指定分类 → 直接写入
  - **例外**：使用子唤醒词（记心愿/记打卡/记情绪）时，分类已确定，跳过确认直接写入

### 搜索笔记
- 触发词：搜备忘、查备忘（别名）
- 子唤醒词：查心愿、查打卡、查情绪（自动带 `-c 分类` 过滤）
- 命令：`script/memo_cli.py search "关键词" -c 分类`

### 更新笔记
- 触发词：改备忘
- 子唤醒词：改心愿、改打卡、改情绪（先按分类搜索，再更新）
- 先搜索找到笔记 ID，再更新
- 命令：`script/memo_cli.py update <id> --content "新内容"`

### 删除笔记
- 触发词：删备忘
- 子唤醒词：删心愿、删打卡、删情绪（先按分类搜索，再删除）
- 先搜索找到笔记 ID，再删除
