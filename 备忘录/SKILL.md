...
**触发场景**:任何生成 HTML 的命令(13 个会触发的命令,见 §触发词速查表)
...
## 用户原话 → 触发词 反向指引表(v1.1.4 · FAT G3 修复)

**目的**:消除"用户口语化表达 → 哪个触发词"的歧义。FAT 实测 AI 缺乏反查表时易误判。

**第一原则**(优先级):
1. **意图优先**:用户语义决定触发词,不靠字面死匹配
