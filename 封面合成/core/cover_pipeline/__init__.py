"""
封面合成 scripts/compose.py
③ 业务层:主流程编排

流程:
1. 校验参数(validators.py)
2. 创建画布(lib/canvas.py)
3. 算布局位置(layout.py)
4. 按 z 顺序处理每张图(layers.py rotate_hard + place)
5. 烧文字(layers.py text_layer)
6. 自检诊断(lib/diagnostics.py)
7. 智能保存(lib/canvas.py safe_save)
8. 返回 {status, data, message, warnings}
"""
