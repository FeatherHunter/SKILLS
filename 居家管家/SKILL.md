...
| 24 | 推位置 | 位置推荐 | features/add.md → Step 2.5 | 是（category） |
| 25 | 找位置 | 参考锚定 | features/add.md → Step 2.6 | 是（reference） |
| 26 | 查账号 | 查看账号 | accounts.py → show/list | 是（平台名，无则列全部） |
| 27 | 存账号 | 新增账号 | accounts.py → add | 是 |
| 28 | 改账号 | 更新账号 | accounts.py → show | 是（平台名） |
| 29 | 查异常 | 数据健康检查 | SKILL.md → Lint 检查 | 否 |
