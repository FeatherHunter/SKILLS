...
| 25 | 推位置 | 位置推荐 | features/add.md → Step 2.5 | 是（category） |
...
| 31 | 查物品-html | 物品搜索→HTML | features/search.md → Step 4 | 可选（无则列全部） |
| 32 | 看物品-html | 物品详情→HTML | features/search.md → Step 4 | 是（多件时先选） |
| 33 | 统物品-html | 总体统计→HTML | features/search.md → Step 4 | 否 |
...
| 查过期 | `stats --type expiring [--days 30] [--expired-only] [--category XX]` |
| 看标签 | `tag-list` |
| 合标签 | `tag-merge --from "旧" --to "新"` |
| 查快递 | `search --status "快递中"` |
| 推位置 | `suggest-locations --category "XX" [--with-examples]` |
...
| 查物品-html | `search --name "XX" --json` |
| 看物品-html | `detail --id {ID} --json` |
| 统物品-html | `list --json` |
...
| 查物品-html | 物品搜索→HTML | features/search.md |
| 看物品-html | 物品详情→HTML | features/search.md |
| 统物品-html | 总体统计→HTML | features/search.md |
