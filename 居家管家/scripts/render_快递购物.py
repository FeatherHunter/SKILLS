# render_快递购物.py - SM5 域渲染助手(08-HTML 交互规范 v1)
...
# 与 render_物品.py 同构(域内自持;共享组件演进 = 公共层 ISSUE)
import json
from datetime import datetime

from render import render_page

SKILL_VERSION = "v2.0-SM5"
...
def build_envelope(data, scene_id, wake_word, command_cn, target=None,
    ...
    """组装标准 payload 信封(与 render_物品.py 同构)"""
    occurred = now_str()
    copy_data = {
        "scene_id": scene_id,
    ...
    copy_log_full = {
        ...
        "call_chain": log.get("call_chain", "scripts/快递购物/cli.py(完整,可复制执行)"),
...
def emit_error(wake_word, command_cn, reason, key_data=None, suggest=None,
    ...
    result = render_page("快递购物/error.html", envelope, output_path, f"{command_cn}失败")
    print(json.dumps(result, ensure_ascii=False))
