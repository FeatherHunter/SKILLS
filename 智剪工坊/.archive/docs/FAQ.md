...
**修复:** 改用 "先生成视觉 + 后叠中文"两步法,见 `scripts/ai_cover.py`。
...
A: 调小强度:
```bash
python scripts/ai_beauty.py --input v.mp4 --output v.mp4 --preset slight
# 或
...
# 先生成多段
for i in {1..3}; do
  python scripts/ai_text_to_video.py --prompt "..." --out "part_$i.mp4"
done
# ffmpeg 拼接
...
**修复:** 剪切时强制 `fps=30`,所有片段统一。`scripts/video_trim.py` 默认已加。
...
**修复:** 改用"先生成视觉(无文字)+ 后叠中文(PIL)"两步法,见 `scripts/ai_cover.py`。
