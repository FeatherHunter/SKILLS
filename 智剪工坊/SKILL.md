...
4. 跑 `references/_check_refs_consistency.py`(如果有)确认无失效引用
...
- **反模式 1：CLI 不存在就自己写** — `scripts/` 里没有的子技能（如 `video_xxx.py`），AI 不得用 `subprocess.run(['ffmpeg', ...])` 直接拼命令行。**正确做法**: 明说"智剪工坊当前不提供 X CLI, 可选方案: (a) 找用户补 CLI (b) 用 ffmpeg 命令行 + 用户接受 (c) 换工具"
