...
## ⚠️ muted 视频拼接风险(v1.10 新增)

**核心问题**:用 `-an`(去除音频)处理过的 mp4,可能残留 audio metadata 在 moov atom 中。这会导致后续 `trim.py concat` 时:
...
**v1.10 自动处理**:`trim.py concat` 已加 pre-process,自动检测并清理残留 metadata。
...
| `references/主流程-阶段编排.md` | 阶段0-5主体流程 | **选①时必读** |
...
|---|---|---|---|
| 01 cutting | 剪头/剪尾/裁切/分段/pin-range/cut-middle | `scripts/video/trim.py` (旧 `processing.py` v1.7 改名 `lib/video_processing.py`) | `01-cutting.md`, `04-cut.md` |
| 02 transitions | 转场/淡入/擦除/滑动 | `scripts/video/xfade.py` | `02-transitions.md` |
| 03 effects | 调色/字幕/文字叠加 | `scripts/video/color.py` / `scripts/video/opening.py` | `03-effects.md`, `05-color.md`, `06-text.md` |
| 04 cinematic | 推镜头/慢动作/倒放 | `scripts/video/speed.py` + `scripts/video/reverse.py` (旧 `processing.py` v1.7 改名 `lib/video_processing.py`) | `04-cinematic.md` |
| 05 image | 图片转视频/Ken Burns | `scripts/video/image_to_video.py` | `05-image.md` |
| 06 text | 字幕/封面文字/数字人 | `scripts/video/opening.py` + `scripts/ai/digital_human.py` | `06-text.md` |
| 07 audio-mix | BGM/混音/淡入淡出/4 mode | `scripts/audio/mix.py` | `07-audio.md` |
| 07 audio-voice | 变声/老声/童声/机器人 | `scripts/audio/voice.py` | `07-audio.md` |
| 07 audio-beat | 节拍/卡点 | `scripts/audio/beat.py` | `07-audio.md` |
| 07 audio-extract | 提取音频/音频提取 | `scripts/audio/extract.py` | `07-audio.md` |
| 07 audio-denoise | 音频降噪/降噪/噪声处理 | `scripts/audio/denoise.py` | `07-audio.md` |
| 07 audio-separate | 声源分离/提取人声/人声分离 | `scripts/audio/separate.py` (v1.7 调 `lib/separate_demucs.py` GPU) | `07-audio.md` |
| 07 audio-voice-extract | 人声提取/纯人声/对话增强 | `scripts/audio/voice_extract.py` | `07-audio.md` |
| 07 audio-silence-split | 静音检测/自动分段/说话人切分 | `scripts/audio/silence_split.py` | `07-audio.md` |
| 07 audio-loudness-norm | 响度归一/音量统一/EBU R128/LUFS | `scripts/audio/loudness_norm.py` | `07-audio.md` |
| 07 audio-diarize | 说话人分离/区分说话人/多人对话/谁说了什么 | `scripts/audio/diarize.py` (v1.7 调 `lib/asr/pyannote.py` 需 HF token) | `07-audio.md` |
| 08 cover | 封面生成（ai/text/image）| `scripts/ai/cover.py` | `08-cover.md` |
| 09 ai-features | 美颜/磨皮/瘦脸/AI 增强 | `scripts/ai/beauty.py` + `scripts/video/color.py` | `09-ai-features.md`, `12-beauty.md` |
| 10 batch | 批量剪辑/流水线 | `scripts/batch/batch.py` | `10-batch.md` |
| 11 asr | ASR/语音转文字/字幕识别/Whisper | `scripts/asr/transcribe.py` (v1.7 调 `lib/asr/whisper.py` GPU) | `11-asr.md` |
| 11 asr-burn | 烧字幕/字幕烧录 | `scripts/asr/burn_subtitle.py` (v1.6 调 `lib/ffmpeg/video/subtitle.py`) | `11-asr.md` |
| 11 asr-speaker | 带说话人的字幕/说话人文字稿 | `scripts/asr/speaker_srt.py` | `11-asr.md` |
| 12 beauty | 美颜/磨皮/瘦脸/大眼 | `scripts/ai/beauty.py` | `12-beauty.md` |
| 13 rewrite-audio | 改词/配音/换声/翻唱 | `scripts/ai/rewrite.py` | `13-rewrite-audio.md` |
| 14 text-to-video | 文字成片/AI 生成视频 | `scripts/ai/text_to_video.py` | `14-text-to-video.md` |
| 15 digital-human | 数字人/AI 讲解/虚拟人 | `scripts/ai/digital_human.py` | `15-digital-human.md` |
| 16 edit | 旋转/缩放/裁剪/静音/提取音频 | `scripts/video/edit.py` (用 `lib/ffmpeg/video/transform.py`) | `16-edit.md` |

**注意**：维度 11 改为 ASR（v1.4 新增链路）；音频维度（07）按音频链路 L1-L5 细分子项。
...
**⚠️ AI 自写代码的 3 种典型反模式（v1.3 禁止）**

- **反模式 1：CLI 不存在就自己写** — `scripts/{audio,asr,video,ai,batch}/` 里没有的子技能，AI 不得用 `subprocess.run(['ffmpeg', ...])` 直接拼命令行。**正确做法**: 明说"智剪工坊当前不提供 X CLI, 可选方案: (a) 找用户补 CLI (b) 用 ffmpeg 命令行 + 用户接受 (c) 换工具"
- **反模式 2：CLI 失败就自己 hack** — CLI 返回非 0 exit code, AI 不得用 Python 重写 ffmpeg 命令。**正确做法**: 读 CLI 错误日志,告诉用户"CLI 在 X 处失败, 错误是 Y, 建议检查 Z"
- **反模式 3：CLI 缺功能就扩展** — CLI 不支持某参数, AI 不得自己 fork CLI 加功能。**正确做法**: 告诉用户"CLI 当前不支持 X, 候选: (a) 提需求补 CLI (b) 用更通用的 X CLI 近似"

### 0. **AI 是编排者, 不是 step 脚本的调用者**（v1.3 关键变化）

- ❌ **不要**调 `pipeline_step*.py`（v1.3 已删 6 个 step 脚本）
- ✅ **要**直接调 用户脚本 CLI（`scripts/{audio,asr,video,ai,batch}/*.py` / `lib/video_processing.py`，v1.7 改名）
