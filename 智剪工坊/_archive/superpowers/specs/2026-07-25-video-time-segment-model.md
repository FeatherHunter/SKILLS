...
用户长按拖动 [2, 55]
  ...
  松开弹出"告知弹窗"(只显示,不要求决策):
    ...
    │                                       │
    │  [2.0~55.0] ← 你选的                 │
    │  [0.0~2.0][55.0~60.0] ← 自动补全两端 │
    │                                       │
    │              [确定]                   │
    └─────────────────────────────────────┘
  > 第一次拖框时 timeline 是"整段保留"(无 segments)。
  > 拖动产生 1 个用户段 + 自动补全前后两个用户没动的部分(默认保留)。
  > 弹窗告知拆分结果,用户点[确定]应用。

  用户点 [确定] → timeline 上出现 3 段**全部彩色**(默认全部保留):
    ┌─[0~2]─┬──────[2~55]──────┬──[55~60]──┐
    │  红   │       橙          │    黄     │
    │(补全)│  (本次拖·新段)   │  (补全)   │
    └───────────────────────────────────────┘
...
用户单击 #1 [0.0~2.0] → 弹出操作面板 → [🗑️ 删掉]
  → #1 从 timeline 消失
  → 折叠区"已删段"出现该项:[♻️ 恢复](显示样式 = 灰色 + 删除线 + 时长)
...
阶段 D · 落入已有段内拖动 = 拆段
═══════════════════════════════════════════════════
用户长按拖动 [31, 33] —— 这段完全落在 #2 [2, 55] 内
  → 自动拆段:#2 切成 [2, 31] + [31, 33] + [33, 55]
  → 告知弹窗"产生 3 个时间段"
  → 用户点 [确定]

用户单击新段 [31, 33] → 弹出操作面板 → [🗑️ 删掉]
  → 该段删除

═══════════════════════════════════════════════════
阶段 E · 落入已有段内拖动 = 拆段 + 加 op
═══════════════════════════════════════════════════
用户长按拖动 [20, 30] —— 落在 #2 [2, 31] 内
  → 自动拆段:[2, 31] 切成 [2, 20] + [20, 30] + [30, 31]
  → 告知弹窗"产生 3 个时间段"

用户单击 [20, 30] → 弹出操作面板 → [🔇 静音]
  → 段加静音 op
  → 该段**显示小图标徽章**(左上角静音喇叭图标,代替色彩区分)
  → tooltip 显示:"[20.0~30.0] · 10s / 🎤 静音"

用户长按拖动 [40, 50] —— 落在 [33, 55] 内
  → 自动拆段:[33, 55] 切成 [33, 40] + [40, 50] + [50, 55]

用户单击 [40, 50] → 弹出操作面板 → [⏩ 倍速]
  → 弹因子 slider(0.25x ~ 4x),默认 1.5x
  → 选 2x → 段加速 op + 显示小徽章"2x"

═══════════════════════════════════════════════════
阶段 F · 折叠区双向跳转
═══════════════════════════════════════════════════
用户点击折叠区"#3 [20.0~30.0]"段项
  → timeline 自动 scrollIntoView 到该段位置
  → 该段闪烁 蓝色高亮 2s 后恢复
...
═══════════════════════════════════════════════════
用户点 [保存]
  → collectFormData() **直接序列化** time_segments(单源真相,不含已删段)
  → 已删段由 JS 实时算 [0, duration] - segments 并集 = 已删补集
  → 已删段不入 JSON;折叠区"已删段"列表从内存计算
...
| 播放窗口 + timeline | 卡片主体 | timeline 可长按拖动框选(段本体**不可拖动**) |
| timeline 段(彩虹色) | timeline 上 | 8 色循环 |
| timeline 段(op 徽章) | 段左上 | 小图标:🎤静音 / ⏩2x 加速 / 🔄反转;hover tooltip 显示完整 op 文本 |
| timeline 段(空白区) | 段与段之间 | 长按拖动 = 拆段 / 新建段 |
| 已删段 | **不显示在 timeline**(折叠区"已删段"列表) |
| 框选告知弹窗 | 全局 modal | 只显示拆分结果 + [确定](无取消) |
| 段单击 → 操作面板 | 段下方 inline | 改 label / 加 op / 删除 |
| 段 ↔ 折叠区双向跳转 | 折叠区段项点击 → timeline scrollIntoView + 2s 高亮 |
| 段悬停 → tooltip | 段上方 | 显示 [start~end 时长 + op 摘要] |
| 已删段项 | 折叠区 | 灰底 + 删除线 + [♻️ 恢复] |

### 6.3.1 拖动语义规范(纠错要点)

| 拖动起点 | 拖动落点 | 行为 |
|---|---|---|
| 段间空白区 | 段间空白区 | 新建段(首次) / 落入已有段内 = 拆段 |
| 段本体 | 任何位置 | ❌ **不响应拖动**(段不可拖动) |
| 段边界 | — | ❌ **不响应拖动**(本期不做边界调整,留 Phase M) |
| timeline 轨道两端的空白([0, duration]外) | — | ❌ 不响应拖动 |

**注意**:**长按拖动(threshold = 200ms)与单击分离**;用户短按段 = 单击 = 操作面板;用户长按(>= 200ms)拖动 = 框选。
