...
用户长按拖动 [2, 55]
  ...
  松开弹出"告知弹窗"(只显示,不要求决策):
    ...
    │              [确定]                 │
    └─────────────────────────────────────┘
  用户点 [确定] → timeline 上出现红色彩虹分段:
    ┌─[0~2]─┬─────[2~55]─────┬──[55~60]──┐
    │  灰   │      红        │    灰     │
    │ (灰=已删默认)        │           │
    └───────────────────────────────────────┘
  ▼ 时间段区:
    #1  [2.0~55.0]  53.0s   [改名][加 op][删除]

═══════════════════════════════════════════════════
阶段 C · 点击段 → 操作面板
═══════════════════════════════════════════════════
用户单击段 #1 [2.0~55.0]
  → 下方弹出该段的操作面板(不是 modal,是在段下方 inline 展开):
    ┌─ 时段 [2.0~55.0] ─────────────────────┐
    │ label: [主体内容____]                  │
    │ op: [🔇 静音] [⏩ 倍速] [🔄 反转]     │
    │     [🗑️ 删掉]                         │
...
阶段 D · 删除子区间(自动切段)
═══════════════════════════════════════════════════
用户单击段 [0,2] 或 [55,60] → 操作面板 → [🗑️ 删掉]
  → 该段变红色(高亮 = 删除态),从 timeline 消失
  → 折叠区"已删段"出现该段:[♻️ 恢复]
