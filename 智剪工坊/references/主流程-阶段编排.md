...
- **处理**:`lib/processing.py process_video()` 自动处理所有 ops + 自动 video_normalize
- **输出**:
  - `单视频/video_{idx}.mp4`
...
ffmpeg 编码成功后**必须**调用 `patch_mp4_rotation`(`lib/patch_mp4_rotation.py`)直接改写 mp4 tkhd matrix atom,清除残留的 displaymatrix metadata。
...
| 4.3 封面生成 | 按 `intent.json.cover.prompt` 生成封面(prompt 不明时 AI 必须问)；**封面文字叠加必须调用 `ai_cover.py` 的 `overlay_text()`，禁止自己写 PIL 脚本** | `scripts/ai_cover.py` |
