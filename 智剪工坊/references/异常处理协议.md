...
异常重试失败或持续失败时，写入 `<workspace>/00_智剪/logs/<task_id>_<timestamp>.md` 异常节（完整路径定义见 `references/AI行为日志协议.md` §6）+ chat 通知用户：
...
**降级规则**：
- 降级前必须写入 `<workspace>/00_智剪/logs/<task_id>_<timestamp>.jsonl`（decision 字段记录"降级到 X"，完整路径见 §3 备注）
- 降级后仍失败 → 停下报告
