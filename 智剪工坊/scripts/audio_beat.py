...
用法:
  # 1. 分析 BGM 节拍,输出 JSON
  python beat_sync.py --analyze --bgm music.mp3 --out beats.json

  # 2. 按节拍点自动剪辑视频(在每个 beat 处切)
...
def cut_to_beats(video_path, bgm_path, output_path, mode="cut"):
    ...
    # 简化:用 trim + atempo 对齐到节拍
    # 实际完整实现需要按节拍点逐段 trim 后 concat
    # 这里只做 "整体视频" 按节拍压缩/拉长
    ...
    log_info(f"完整节拍卡点需要逐段 trim + concat,本版本只做整体对齐")
