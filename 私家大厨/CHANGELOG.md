...
用户反馈 4 个体验问题:
1. 「▶ 开始做菜」「🛒 生成采购清单」两个按钮是多余的(违反 5 层架构)
2. 顶部「📋 复制食材清单」无反应
3. 份数 +/− 按钮无反应
4. 步骤食材末尾 `(中途加入)` 显示奇怪

后用户补充需求:这两个 CTA 改为“复制 prompt”模式,复制自然语言指令后粘贴给 AI,触发对应做菜模式 / 采购清单流程。这样保留了跨 Skill 入口,但入口归 AI 控制层,HTML 只做“复制”。

外加数据问题:数据库有 4 条同名「辣椒炒肉」,只应保留 1 条。

### 改动

- `templates/recipe_view.html`
  - 首屏 CTA 改成“复制 prompt”:`📋 复制「开始做菜」指令` / `🛒 复制「采购清单」指令`,复制自然语言指令,粘贴给 AI 后由 AI 调对应脚本生成正式 HTML
  - 三个复制按钮共用 `doCopy()` + 不同 feedback,JS 从 IIFE 改成 `DOMContentLoaded` 监听
  - 步骤食材末尾去掉 `中途加入`(其他引入时机保留)
  - 按第一性原理重排功能区(阅读动线 = 判断 → 决策 → 执行 → 复盘 → 出口 → 存档):
    上次经验 → 决策卡(基本信息 + insight + 营养提醒) → 切配建议 → 食材 + 份数 + 调料 → 步骤(关键成功点 → 时间轴 → 详细) → 技法 → 小贴士 → 炊具 → 派生 → 历史 → 营养表 → 背景 → CTA → footer
  - CTA 三个按钮独立为「继续操作」卡,放在 footer 上方,不再占领首屏
  - 「继续操作」三按钮加 `no-focus-ring` 类,去掉点击/键盘聚焦时的蓝色焦点环
  - 「基础 X 人份」改为 `#serv-base` 元素,份数变化时 JS 同步更新(从 1 改到 4 也显示“基础 4 人份”),后回退为静态「人份(原方 N)」避免显示冗余
  - `.action-btn` 全局加 `:focus/:focus-visible/:active { outline:none; box-shadow:none }`,彻底消除焦点环/点击涟漪
  - 复制反馈文案改成 `✓ 已复制,粘贴给 AI 即可`,失败时显示 `⚠️ 浏览器拦截了复制,请用 Ctrl+C`,显示时长从 1.8s 提到 3.5s,带绿色背景条更醒目
- 数据清理
  - 备份 `D:\2Study\StudyNotes\.db\chef_data.db.bak.20260723_080746_dedup_lajiaochaorou`
  - 删除 3 条重复「辣椒炒肉」(连带 17 张表关联数据共 75+ 条)
  - 保留最早一条 id=230ce6e5-7584-4365-8242-6d4e8b2a166e

### 影响面

只改 `templates/recipe_view.html` + 数据库去重,无 CLI/契约变更。
...
用户检查"辣椒炒肉"做菜模式视觉效果时发现,右侧"当前进度"里的"剩余约 N 分钟"在超时、暂停、重做场景下语义不够稳定。

### 第一性原理

剩余时间不是"数据库总时间"也不是"当前倒计时归零后的 0",而是用户从当前状态到完成整道菜仍需投入的预计烹饪时间。

### 改动清单

- `templates/cooking_mode.html`:抽出 `renderCookSideInfo()`,统一计算右侧当前进度和下一步提示。
- 正常倒计时:当前步骤剩余秒数向上取整 + 后续步骤计划分钟之和。
- 暂停:倒计时不减少,右侧剩余时间保持暂停瞬间值。
- 重做:当前步骤倒计时回到计划时长,右侧剩余时间同步重算。
- 超时:当前步骤最低按该步骤计划时长显示;例如辣椒炒肉第 1 步计划 5 分钟 + 后续 2 分钟,超时后仍最低显示 7 分钟。
- `references/cooking_mode.md` / `SKILL.md` / `私家大厨.html`:同步规则说明。

### 影响面

只改前端模板与文档层,不改数据库 schema,不改 CLI 签名,无数据迁移。
