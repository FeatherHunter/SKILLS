...
### ⚠️ 待拍板路由(4 项 · 边界模糊 · 待用户拍板)

| 用户原话 | 推荐归属 | 待拍板理由 |
|----------|----------|-----------|
| "查看营养 X"(菜本身的营养) | **私家大厨** | recipes.nutrition_info vs 卡路里 nutrition_goal — 需用户明确"看 X 菜的营养"还是"我的营养" |
| "采购清单 + 营养"(联动) | **不联动** | 采购清单是"这次买菜",卡路里食品库是"我常吃的" — 除非用户明确说"顺便存到食品库" |
| "炊具" | **私家大厨**(做菜需要哪些) vs **居家管家**(实物管理) | recipes.cookware 表只管"做菜需要" — "我家有几个炒锅/锅在哪" 归居家管家 |
| "采购下单" | **居家管家** | SKILL.md 第 7 行声明"不管食材采购下单(那是居家管家)" — 需核实居家管家是否实现此功能 |
...
| 唤醒词 | 说明 |
|--------|------|
| 查看食谱 | 调 `recipe_render.py render <菜名>` 推完整 HTML(全部 section) |
| 查看食材 | 推完整 HTML,用户自行定位食材 section |
| 查看步骤 | 推完整 HTML,用户自行定位步骤 section |
| 查看营养 | 推完整 HTML,用户自行定位营养 section |
| 查看背景 | 推完整 HTML,用户自行定位背景 section |
...
| 唤醒词 | 说明 |
|--------|------|
| 私家大厨 HELP | 调 `render_help.py` 渲染 HELP HTML，展示全部 35 业务唤醒词 + 61 场景（§07 §1 强制契约） |
