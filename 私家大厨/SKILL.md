...
**不管什么** —— 不管食材采购下单(用户 2026-07-27 决策:**不实现**,本技能和居家管家都不实现)、不管个人热量与营养记录(那是卡路里)、不管外卖/外送、不管自动烹饪设备。

**一句话**:你的菜,这个本子全管;别的本子管的事,这个本子不碰。

---

## 跨 Skill 路由表(§03 铁律 3 · 显式声明)
...
| 用户原话 | 归属 | 判定依据 |
|----------|------|----------|
| 采购清单 / 生成清单 / 排除可选 | **私家大厨** | shopping_manager.py 已实现 |
| 录入食谱 / 导入食谱 | **私家大厨** | recipe_import.py + orchestrator |
...
| 用户原话 | 归属 | 边界判定 |
|----------|------|---------|
| "查看营养 X"(菜本身的营养) | **私家大厨** | 看 recipes.nutrition_info 表(每道菜独立营养);若用户说"查营养"或"我的营养" → 卡路里 |
...
- **"采购下单"** —— 用户决策:**不实现**。本技能不实现,居家管家也不实现。SKILL.md 第 7 行"不管食材采购下单(那是居家管家)"声明**作废** —— 改为"不管食材采购下单(用户决策:不实现)"。

**拍板原则**:按"用户说 X 实际指向的实体归属"判定,而不是按"哪个 skill 有类似功能"判定。
...
> 总览词和细分词并列共存，互不冲突。
>
> **变体方向(§03 变体管理 · 2026-07-27 增):** 每个核心词配 2-3 个变体,3 方向:
> - **S · 同义** — 换说法(查询/找一下/搜下)
> - **O · 口语** — 加口语化 token(帮我/给我看下/呢/哈/一下)
> - **F · 模糊** — 缺主语/语法略错/同音字(那 X 怎么找/X 走起/X 错字)
>
...
|--------|------|
| 做菜模式 | 进入做菜模式，需配合菜名 | - | - | - |
| 开始做菜 | "开始做宫保虾球" | ✓ 开火做/下厨 | ✓ 帮我下厨哈/给我做 | ✓ 走起/搞/整 |
...
| 唤醒词 | 说明 | 同义(S) | 口语(O) | 模糊(F) |
|--------|------|
| 搜索食谱 | 按菜名关键词搜索 | ✓ 找一下/查/搜下 | ✓ 帮我找 X 哈 | ✓ X 的菜有? |
| 筛选菜系 | "筛选川菜" | ✓ 川菜/粤菜 | ✓ 有啥川菜 | - |
| 筛选食材 | "筛选含虾的菜" | ✓ 含 X 的/有 X | ✓ 帮我找含 X | - |
| 筛选难度 | "筛选简单菜" | ✓ 简单的/好做的 | ✓ 来个简单的 | - |
| 筛选时间 | "筛选30分钟内的菜" | ✓ 30分钟内/快的 | ✓ 几分钟能搞 | - |
| 筛选炊具 | "筛选用砂锅的菜" | ✓ 用 X 做的 | ✓ 帮我用 X 做的 | - |
| 筛选口味 | "筛选辣的菜" | ✓ 辣的/甜的 | ✓ 口味 X 味的 | - |
| 筛选季节 | "筛选适合夏天的菜" | ✓ 夏天/冬天吃 | ✓ 夏天吃啥 | - |
| 筛选状态 | "筛选已做的菜" | ✓ 做过的/没做过的 | ✓ 我做过哪些 | - |
| 查看全部 | 列出所有食谱，无筛选条件 | ✓ 全部/所有 | ✓ 都有啥 | ✓ 多少菜 |

### D. 修改食谱（9个）

| 唤醒词 | 说明 | 同义(S) | 口语(O) | 模糊(F) |
|--------|------|
| 修改食谱 | 通用修改入口 | ✓ 改 X | ✓ 帮我改 X | - |
| 修改步骤 | 改步骤内容/顺序 | ✓ 改 X 的第 N 步 | ✓ 第 N 步改下 | - |
| 修改食材 | 改食材用量/添加食材 | ✓ 加/减 X | ✓ 帮我改下 X | - |
| 修改难度 | 改难度等级 | ✓ 改难度/难易 | ✓ 难度调下 | - |
| 修改份量 | 改份数 | ✓ 改份数/几人份 | ✓ 改下份数 | - |
| 废弃食谱 | 标记为已废弃（只增不删） | ✓ 不要 X | ✓ X 不要了 | - |
| 不想要 | (aliases 共享场景) | - | - | - |
| 删掉 | (aliases 共享场景) | - | - | - |
| 废弃 | (aliases 共享场景) | - | - | - |
...
| 唤醒词 | 说明 | 同义(S) | 口语(O) | 模糊(F) |
|--------|------|
| 生成清单 | 生成采购清单 | ✓ X 要买啥/购物清单 | ✓ 帮我整下单子 | ✓ X 采购 |
| 排除可选 | 生成不含可选食材的清单 | ✓ 不要可选/就买必须的 | ✓ 不要可选项 | - |
...
| 唤醒词 | 说明 | 同义(S) | 口语(O) | 模糊(F) |
|--------|------|--------|--------|--------|
| 私家大厨 HELP | 调 `render_help.py` 渲染 HELP HTML，展示全部 35 业务唤醒词 + 61 场景（§07 §1 强制契约） | - | ✓ 菜谱 HELP/能做什么 | ✓ 这能做啥 |
...
### 全字段必填（硬规则 · 无跳过通道 · v4.0 缺字段拒绝制）

> **录入食谱时,所有字段都必须有真实值:不允许 `null`,也不允许空字符串(2026-08-09 用户拍板)。AI 不得"字段不重要就跳过"。**

1. **JSON 必须包含所有字段且值必须为真实数据** — 不允许 `null`、不允许 `""`(空字符串);缺了 = 校验拒绝
2. **缺字段 = 校验失败,一次列全** — `scripts/validators.py::validate_missing_manifest()` 把 null/空串/占位符/步骤关联失效**一次列全**成缺失清单,AI 收到后必须逐项问用户补齐(早失败,不在写库阶段连环炸)
3. **可显式 null 仅限 DB 可空字段,且不在 JSON 契约内** — `tips.step_id / tips.ingredient_id` 由写库层从 `step_sequence` 派生,JSON 不携带;`steps[].ingredients_used[].unit` 缺时由 `ingredients[].unit` 兜底,两个都没有才报缺失
4. **多个字段缺失时,一次性问用户** — 用一个问题把多个字段打包问,不要每个字段单独问(那太烦)
5. **无真实值(如无照片)时,AI 必须问用户** — 用户显式确认后才可用占位图并注明(非系统自动填)
6. **错误信息含"字段名 + 当前值 + 期望值 + 怎么修"** — 遵守"可约束"特性,让 AI 知道怎么改
7. **早失败优于晚失败** — 在校验阶段直接拒,不要等到 SQL 报错
8. **无 --force / --skip-validation 通道** — AI 偷懒会立刻用上,设计上禁止
...
第3步：功能分流
    ...
    细分唤醒词（如"查看食材"）→ 告知 AI 推完整 HTML(原则 11 HTML-First),**禁止文字截取**,用户在 HTML 里自行定位
    菜名缺失 → 追问用户（不能猜）
    ↓
第4步：兜底
...
|---|--------|------|
| 1 | 做菜模式 | 进入做菜模式，需配合菜名 | - | - | - |
| 2 | 开始做菜 | "开始做宫保虾球" | ✓ 开火做/下厨 | ✓ 帮我下厨哈/给我做 | ✓ 走起/搞/整 |
...
| # | 唤醒词 | 说明 |
|---|--------|------|
| 8 | 搜索食谱 | 按菜名关键词搜索 | ✓ 找一下/查/搜下 | ✓ 帮我找 X 哈 | ✓ X 的菜有? |
| 9 | 筛选菜系 | "筛选川菜"，按菜系筛选 |
...
| 17 | 查看全部 | 列出所有食谱，无筛选条件 | ✓ 全部/所有 | ✓ 都有啥 | ✓ 多少菜 |
...
| 19 | 修改步骤 | 改步骤内容/顺序 | ✓ 改 X 的第 N 步 | ✓ 第 N 步改下 | - |
| 20 | 修改食材 | 改食材用量/添加食材 | ✓ 加/减 X | ✓ 帮我改下 X | - |
| 21 | 修改难度 | 改难度等级 | ✓ 改难度/难易 | ✓ 难度调下 | - |
| 22 | 修改份量 | 改份数 | ✓ 改份数/几人份 | ✓ 改下份数 | - |
...
| # | 唤醒词 | 说明 |
|---|--------|------|
| 29 | 私家大厨 HELP | 调 `render_help.py` 渲染 HELP HTML，输出 `$CHEF_OUTPUT_DIR/help/私家大厨_HELP_<时间戳>.html`，展示全部 35 业务唤醒词 + 61 场景（HELP 自身不出现在该 HTML 中） |
...
| 28 | 排除可选 | 生成不含可选食材的清单 | ✓ 不要可选/就买必须的 | ✓ 不要可选项 | - |
...
| 30 | 导入食谱 | JSON文件导入 | ✓ 导入 JSON | ✓ 帮我导入下 | - |
