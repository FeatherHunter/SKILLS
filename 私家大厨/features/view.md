...
| # | 约束 | 说明 |
|---|------|------|
| C1 | 页面结构 | 2种页面：首页（第1页）、步骤页（每步1页） |
| C2 | 线性步骤 | 步骤按顺序1→2→3→N执行，不能跳过 |
| C3 | 用户触发切换 | 完成当前步骤后，点"完成步骤→"手动触发跳转到下一步骤页 |
| C4 | 依赖检查 | 每步解锁前AI判断：上一步完成？上一步等待型？等待到期？三题全过才解锁 |
| C5 | 等待型步骤页 | 若解锁的步骤是等待型，进入等待页，显示倒计时，用户确认后才继续 |
| C6 | 顶部计时器固定区 | 所有进行中的计时器（最多4个）固定在页面顶部，超出横向滚动 |
| C7 | 并发等待 | 多个等待步骤可同时计时（炖30分钟同时腌15分钟），各计时器独立 |
| C8 | 到期高亮 | 等待到期时该步骤高亮（橙色背景），左上角闪烁提示条，持续5秒 |
...
canUnlock(step N):
    if step N-1 is not completed:
        return false  // 上一步未完成
    if step N-1 is wait-type:
        if wait timer not expired:
            return false  // 等待未到期
    return true  // 三题全过，解锁
```
...
- 固定在页面顶部，不随步骤切换消失
- 最多显示4个计时器，超出横向滚动
- 每个计时器显示：步骤编号 + 剩余时间（MM:SS）
- 到期计时器橙色高亮，持续闪烁直到用户确认
...
| 属性 | 值 |
|------|-----|
| 背景色 | `rgba(255, 152, 0, 0.2)` 橙色半透明 |
| 边框 | `2px solid #ff9800` |
| 显示文字 | "⏰ 时间到！请点击继续" |
| 持续条件 | 直到用户点击步骤上的"继续"按钮 |
| 页面提示 | 左上角出现闪烁提示条（持续5秒） |
