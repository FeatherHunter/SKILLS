...
AI 必须通过 `scripts/cooking_render.py render <菜名或ID>` 生成注入后的副本文件,不得直接把未注入的 `templates/cooking_mode.html` 当正式交付。

```html
<media src="$CHEF_OUTPUT_DIR/cooking/做菜模式_<recipe_slug>_<YYYYMMDD_HHMMSS>.html" type="file" />
```

文件命名规则:

- 输出目录:`$CHEF_OUTPUT_DIR/cooking/`
- 文件名:`做菜模式_<recipe_slug>_<YYYYMMDD_HHMMSS>.html`
- 示例:`做菜模式_辣椒炒肉_20260723_183000.html`
- 禁止:`cooking_mode.html`、`cooking_mode_辣椒炒肉_fixed.html`、`preview.html`、`test.html` 作为正式交付名
...
做饭页右侧"当前进度"的"剩余约 N 分钟"按第一性原理定义为:用户从当前状态到做完整道菜还需要投入的预计烹饪时间。

计算公式:

```text
剩余约 N 分钟 = 当前步骤显示剩余分钟 + 后续所有步骤计划分钟之和
```

当前步骤显示剩余分钟规则:

- 正常倒计时中:用当前步骤倒计时秒数向上取整,例如 4:01 显示 5 分钟。
- 暂停时:倒计时不减少,剩余时间保持暂停瞬间的值。
- 重做时:当前步骤倒计时重置为该步骤计划时长,剩余时间同步回到"当前步骤完整时长 + 后续步骤总时长"。
- 倒计时归零或超时时:当前步骤最低按该步骤计划时长显示,不因为 00:00 或超时变成 0;例如第 1 步计划 5 分钟、后续 2 分钟,归零/超时后仍最低显示 7 分钟。
- 切换下一步/上一步:当前步骤按新步骤计划时长重新计算,后续步骤随当前序号变化。
...
AI 加载本技能 + 本文档
  ↓
AI:python scripts/cooking_render.py render 辣椒炒肉
  ↓
脚本:show --json + 注入 templates/cooking_mode.html + 输出正式 HTML
  ↓ (<media src="..." />)
用户在 HTML 里:
  阶段 1 看准备清单 → 切配 → 点"开始"
