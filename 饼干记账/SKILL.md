...
1. **`饼干记账.html` = render_help.py 命令生成的 HELP 镜像** — 生成后自动复制到技能根目录,不再手工同步;改动 HELP 相关代码/模板后必须重新生成,一致性由 tests/test_render.py 契约锁定
2. **修改确认采用任务级授权** — 用户显式点名/下发的任务(如 ready-for-agent 票 / wayfinder ticket)视为对任务范围内修改的一次确认,执行中不再逐行确认;任务范围外、涉及真实数据或破坏性变更仍需先确认
...
> G5 Q3 决议落地（#249）· 相对时间换算细则的唯一承载章节；Step 2 括号引用收敛于此。

**触发识别总则（G5 决议 · 变体方向制）：** 触发识别职责 = frontmatter description 触发词矩阵（按域分组）+ AI 自然语言理解（同义 / 口语 / 模糊 3 方向），不再硬编码别名映射表；唤醒词 = AI 路由入口（简短），场景名 = HELP 阅读（「做什么 +（怎么处理）」）。相对时间表达由 AI 按本表语义换算为具体日期后传入 CLI 参数（CLI 始终接收具体日期）。

**可执行参考实现：** `scripts/routing.py`（3 函数 + today 注入，纯库不进运行时）+ `tests/test_routing.py`（40 用例，覆盖跨月/跨年/闰年/12 月/跨年+1）。**修改本表规则必须同步 routing.py，反之亦然。**
...
|------|----------|--------------|
| 今天 | 当天 | `--date 今天` |
| 昨天 | 前一天（跨月/跨年自动进位） | `--date 昨天` |
| 前天 | 前两天 | `--date 前天` |
| N 天前 | 当天 − N 天 | `--date 3天前` |
| 本周五 / 上周五 / 上上周五 | 所在周偏移 + 周几（本周=0 / 上周=−1 / 上上周=−2，周一=1…周日=7） | `--date 上周五` |
| 本周 | **周一 ~ 周日**（含今天所在周） | `--from 本周一 --to 今天` |
| 上周 | 上周一 ~ 上周日 | `--from 上周一 --to 上周日` |
| 本月 | 本月 1 日 ~ 月末 | `--month 本月` |
| 上月 | 上月 1 日 ~ 月末（1 月 → 去年 12 月） | `--month 上月` |
| 最近 N 天 | 今天 − (N−1) ~ 今天（含今天） | `--days N` |

**关键语义声明：**
- 「本周」= 周一 ~ 周日（ISO 语义），自持不共享
- 跨月/跨年/闰年/12 月/跨年+1 等边界由 `tests/test_routing.py` 锁死（如「昨天」= 1 月 1 日 → 去年 12 月 31 日；「上月」= 2024-03 → 2024-02-01 ~ 02-29）
...
**路由识别（G5 决议 · 变体方向制）：** 触发识别职责 = frontmatter description 触发词矩阵（按域分组）+ AI 自然语言理解（同义 / 口语 / 模糊 3 方向），不再硬编码别名映射表；唤醒词 = AI 路由入口（简短），场景名 = HELP 阅读（「做什么 +（怎么处理）」）。相对时间表达（今天 / 昨天 / 上周五 / 最近 N 天等）的换算细则见 §路由规则。
...
1. 通过 `查最近` / `查某天` / `搜备注` 定位目标记录的 ID(必要时把候选展示给用户挑)
